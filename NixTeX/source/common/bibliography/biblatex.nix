...
let
  ...
  formatAuthors =
    let
      formatParticle = particle: if particle != null then "${particle} " else "";
      formatSuffix = suffix: if suffix != null then ", ${suffix}" else "";
    in
  ...
  formatPages =
    pages: if set.isInstanceOf pages then formatPagesFromTill pages else "pages = {${string pages}}";

  formatPagesFromTill =
    { from, till }:
    if from != till then "pages = {${string from}–${string till}}" else "pages = {${string from}}";
  ...
  getType =
    ...
    .${variant} or (debug.panic "getType" "Invalid variant »${variant}«");

  getFormatter =
    shortJournalNames: variant:
    let
      JournalArticle =
        { issue, pages, ... }:
        let
          journalName = if shortJournalNames then issue.journal.short else issue.journal.name;
        in
    ...
    .${variant} or (debug.panic "getFormatter" "Invalid variant »${variant}«");
  ...
  toLine = name: value: if value != null then "${name} = {${string value}}" else null;
