...
let
  ...
  parseUnit # :  string -> Unit
    =
      ...
      if unit == "cm-1" then
        ...
        debug.panic "parseUnit" {
          data = unit;
          text = "Cannot parse unit:";
        };
  ...
  formatUnit # :  U: ToUnit @ [ U ] | U -> string
    =
      ...
      in
      if text != null then "\\ensuremath{${text}}" else "";

  formatUnit' # :  U: ToUnit @ [ U ] | U -> string
    =
      maybeUnit:
      if list.isInstanceOf maybeUnit then
        string.concatMappedWith formatUnit' "\\cdot" maybeUnit
      else
        let
          unit = Unit maybeUnit;
          prefix = if unit.prefix != null && unit.prefix != "" then "\\acrshort{${unit.prefix}}" else "";
          link =
            {
          ...
          formatted = "\\text{${prefix}${link}}";
          withNumericExponent =
            if unit.exp < 0 then
              "${formatted}^{-\\text{${number.toSignificantString (0 - unit.exp)}}}"
            else
              "${formatted}^{\\text{${number.toSignificantString unit.exp}}}";
        ...
        if unit != null then
          type.matchPrimitiveOrPanic unit.exp {
            ...
            string = "${formatted}^{${sign}${adjustNumbers unit.exp}}";
          }
        else
          null;
      ...
      type.matchPrimitiveOrPanic value {
        string =
          ...
          if parsed != null then
            {
              value =
                (integer "0${list.get parsed 1}") + ((integer "0${list.get parsed 2}") / (number.pow 10 precision));
              exp =
                let
                  exponent = list.get parsed 4;
                  sign = list.get parsed 3;
                in
                if exponent != "" then "${if sign == "-" then "-" else ""}${exponent}" else "";
              inherit precision;
        ...
        set =
          if value ? from && value ? till then
            ...
            {
              value = type.matchPrimitiveOrPanic value.value {
                string = value.value;
                float = value.value;
                int = value.value;
                set =
                  if value.value ? from && value.value ? till then
                    value.value
                  else
  ...
  formatValue # :  V: ToValue, U: ToUnit @ V -> U -> string
    =
      value: unit: "\\mbox{\\ensuremath{${formatValueInMath value unit}}}";
  ...
  formatValueInMath # :  V: ToValue, U: ToUnit @ V -> U -> string
    =
      ...
      in
      if unitText != null then "${valueText}\\,${unitText}" else valueText;
  ...
  formatValue' # :  V: ToValue @ [ V ] | V -> string
    =
      ...
      let
        ...
        valueText = type.matchPrimitiveOrPanic value'.value {
          set =
            let
              text = number.toStringWithPrecision value'.value value'.precision;
            in
            if value'.exp != null || value'.sign != null then "(${text})" else text;
          string = "${value'.value}";
          float = numeric;
          int = numeric;
          list =
            let
              text = string.concatMappedCSV (
                { precision, ... }@value:
                formatValue' (if precision != null then value // { inherit precision; } else value)
              ) value'.value;
            in
            if value'.exp != null || value'.sign != null then "(${text})" else text;
        };
        exponentText =
          ...
          in
          if exponent != null then "\\cdot\\text{10}^{${exponent}}" else "";
      in
      "${formatSign value'.sign}${adjustNumbers valueText}${exponentText}";

  adjustNumbers =
    this:
    string.concatMapped (part: if list.isInstanceOf part then "\\text{${list.head part}}" else part) (
