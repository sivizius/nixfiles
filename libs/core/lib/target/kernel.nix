  ...
  Kernel # struct { executableFormat: ExecutableFormat, family: Family }
    = type "Kernel" {
      ...
      isDarwin # :  System -> bool
        =
          { kernel, ... }@self: kernel == all.darwin;

      isLinux # :  System -> bool
        =
          { kernel, ... }@self: kernel == all.linux;

      from # :  type -> (string | Kernel)? -> Kernel
        =
          kernel:
          if !(Kernel.isInstanceOf kernel) then
