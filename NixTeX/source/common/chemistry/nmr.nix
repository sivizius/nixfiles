...
let
  ...
  adjustSignals/* :  [ string ] -> [ string ] */ = signals:
    let
  ...
  # LaTeX
  formatNMRnucleus/* :  string -> string */ = nucleus:
    let
      tex = {
  ...
  # LaTeX
  formatSpectrum/* :  Spectrum -> Chunk */ = { acronyms, ... }:
  ...
  genDependencies/* :  Product -> Dependencies */ = { substance ? { }, identifier ? null, nmr ? null, name ? null, ... }:
    let
      nmr' = substance.nmr or nmr;
      files = list.concatMap
        (
          spectrum:
          if spectrum != null
          then
  ...
  # LaTeX
  mapSignals/* :  string -> [ Signal ] -> [ string ] */ = { acronyms, ... }:
    nucleus:
    list.map
      (
        ...
        let
          ...
          details = (if false then [ protons' ] else [ ])
  ...
  # LaTeX
  reportSpectra/* :  NMRdata -> [ LaTeX ] | ! */ = { acronyms, ... } @ resources:
    { kind, nmrData }:
    let
