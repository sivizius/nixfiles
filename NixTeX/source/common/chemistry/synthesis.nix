...
let
            ...
            then
              ...
              then
                ...
                [
                  ...
                  "    (${physical.formatValue { inherit(amount') value; precision = 2; } "${amount'.prefix}mol"})"
  ...
  formatProductChemicals = resources:
    syn:
    product:
    ...
    map
      (
        ...
        let
          ...
          body = (toList (formatProductChemicals resources synthesis product'))
            ++ (
            if note != null
            then
  ...
  fixChemical = { acronyms, ... } @ resources:
    chemicals:
    name:
        ...
        let
          ...
          product =
            ...
            then
              ...
              [ ];
          citeLiterature = { literature ? null, ... }:
    ...
    let
      ...
      ++ (
        ...
        then
          let
            ...
            volume' = physical.formatValue { inherit (volume) value; inherit precision; } "${volume.prefix}litre";

            fullVolume = normaliseValue (chemical.times * chemical.volume / 1000.0) null;
            fullVolume' = physical.formatValue { inherit (fullVolume) value; inherit precision; } "${fullVolume.prefix}litre";
          in
      ...
      ++ (
        ...
        then
          ...
          in
          [ "${( physical.formatValue { inherit(mass) value; precision = 2; } "${mass.prefix}gram" )}" ]
        else
          [ ]
      )
      ++ (
  ...
  getUnitPrefix/* :  integer -> string | ! */ = factor:
  ...
  getTitle = { acronym ? null, mixture ? null, substance ? null, title ? null, ... }:
  ...
  formatConcentration = concentration:
    ...
    then
      "${( physical.formatValue { inherit(concentration') value; precision = 1; } "${concentration'.prefix}molar" )}"
    else
