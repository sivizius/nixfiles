...
let
  ...
  fullTable = set.map (symbol: value: value // { inherit symbol; }) {
    # Period 1
    H = {
      dalton = 1.0080;
    };
  ...
  normaliseMolecularFormula =
    let
      ...
      normalise = this: finalise (list.fold parse defaultState this);
    in
        ...
        type.matchPrimitiveOrPanic formula {
          ...
          set = set.fold (
            ...
            (
              ...
              +
                count
                * (tableOfMasses.${symbol} or (debug.panic "calculateMassOfFormula" "Unknown symbol: ${symbol}"))
            )
          ) 0 formula;
        }
      );
      ...
      let
        ...
        calculated =
          let
            ...
            filtered = set.map (symbol: { ... }: mapped.${symbol} or 0.0) gotElements;
