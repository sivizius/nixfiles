...
let
  ...
  matchNumber # :  T -> { int: R, float: R } -> R | !
    ...
    =
      value:
      { int, float }@select:
      type.matchPrimitiveOrDefault value select (
  ...
  isInstanceOf = value: integer.isInstanceOf value || float.isInstanceOf value;
  ...
  pow # :  int -> int | float -> int | float
    =
      let
        pow # :  int -> int | float -> int | float
          =
            base: exp: list.fold (y: x: x * y) 1.0 (list.generate (x: base) exp);
      in
      base: exp: if exp < 0.0 then pow (1.0 / base) (0 - exp) else pow (1.0 * base) exp;
  ...
  toInteger' # :  string -> int | null
    =
      ...
      in
      if value' != null && integer.isInstanceOf result then
      ...
      if precision > 0 then
        list.foldReversed (
          ...
          if state != null then
            ...
            null
        ) null (string.toCharacters (toStringWithPrecision value precision))
      else
  ...
  toStringWithPrecision # :  int | float -> int -> string
    =
      ...
      type.matchPrimitiveOrDefault value {
        ...
        set =
          ({ from, till }: "${toStringWithPrecision from precision}–${toStringWithPrecision till precision}")
            value;
      } (debug.panic "toStringWithPrecision" "Value must be a numeric value like int or float!");

  toSignificantString # :  int | float -> string
    =
      value: toStringWithPrecision value null;

  getPrecision =
    value:
    debug.info "getPrecision" {
      text = "value";
      data = value;
    } (if value == 0.0 || value >= 2.0 || value <= -2.0 then 0 else (getPrecision (value * 10)) + 1);
