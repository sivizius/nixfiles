...
let
  ...
  formatUnitDescription =
    ...
    let
      optional = condition: text: if condition then [ text ] else [ ];
      origin =
      ...
      value' =
        let
          ...
          mapValues =
            ValueList:
            "\\mbox{\\ensuremath{${
              string.concatMappedWith ({ value, unit }: formatValueInMath value unit) " = " ValueList
            }}}";
        in
      ...
      parts =
        (
          ...
          else if siDerived then
            [ "\\acrshort{siStandard}-Einheit ${about}" ]
          else if pseudoUnit then
            [ "Pseudo\\-einheit" ]
          else if archaic then
