...
let
  ...
  buildTextFile = { body, fileName, buildSystem, name, permissions, registries }:
    let
  ...
  buildDirectory = { body, buildSystem, name, registries }:
    let
      builder = writeScriptFile
        {
          name = "${name}-builder";
  ...
  writeTextFile = path.toFile "writeTextFile.sh"
    ''
      $coreutils/mkdir $out
  ...
  )
{
  ...
  bashScript' = { buildSystem, registries, ... }:
    name:
    body:
    let
  ...
  directory = { buildSystem, registries, ... }:
    name:
    body:
    buildDirectory { inherit body buildSystem name registries; };

  pythonScript = { buildSystem, registries, targetSystem, ... }:
    name:
    body:
    let
  ...
  pythonScript' = { buildSystem, registries, ... }:
    name:
    body:
    let
  ...
  shellScript = { buildSystem, registries, targetSystem, ... }:
    name:
    body:
    let
  ...
  shellScript' = { buildSystem, registries, ... }:
    name:
    body:
    let
