...
let
  ...
  check =
    let
      ...
      optional = condition: body: if condition then [ body ] else [ ];
      check1H =
        name: spectra:
        if spectra."1H" or null != null then
          let
            spectra' = spectra."1H".files;
          in
          [ ]
          ++ (optional (spectra'.self or null == null) "1H-Spectrum for novel ${name} missing")
          ++ (optional (spectra'.cosy or null == null) "1H-COSY-Spectrum for novel ${name} missing")
        else
          [ "1H-NMR for novel ${name} missing" ];
      check13C =
        name: spectra:
        if spectra."13C" or null != null then
          let
      ...
      checkNMR =
        name: nmr:
        if nmr != null then
          [ ] ++ (check1H name nmr) ++ (check13C name nmr)
        else
  ...
  check' = substances: string.concatLines (check (context "check") substances);

  checkNovel =
    substances:
    let
      novelSubstances = list.filter (
        {
          novel ? false,
          ...
        }:
        novel
      ) (set.values substances);
      formattedSubstances = list.map (
        {
          name,
          nmr ? null,
          ir ? null,
        ...
        let
          nmr' = if nmr != null then nmr else "\\multicolumn{4}{c|}{– NMR? –}";
        in
        "${name} & "
      ) novelSubstances;
    in
