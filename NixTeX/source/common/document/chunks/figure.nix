...
let
  ...
  orDefault = first: second: if first != null then first else second;
  ...
  mapSubfigure =
    ...
    // (
      if file != null then
        {
          body = null;
          file = type.matchPrimitiveOrPanic file {
            path = file;
            string = file;
          };
  ...
  mapSubfigures = list.map (config: if config != null then mapSubfigure config else config);
    ...
    if output == "LaTeX" then
      ...
      ++ (
        if subfigures != null then
          ...
          in
          list.concatMap (figure: (if figure != null then convert figure else [ "\\hfill" ])) subfigures
        else
          render figure
      )
      ++ (putCaption figure)
      ++ (if label != null then [ "\\labelFigure{${label}}%" ] else [ ])
      ++ [
        indentation.less
        "\\end{${environment}}%"
      ]
  ...
  choose = list.fold (result: item: if result == null then item else result) null;
in
{
  ...
  Figure =
    ...
    chunks.Chunk "Figure"
      ...
      (
        {
          caption = toCaption (choose [
            caption
            description
            ""
          ]);
          description = toDescription (choose [
            description
            caption
            ""
          ]);
          ...
          if file != null then
              ...
              dependencies =
                ...
                ++ (type.matchPrimitiveOrPanic file {
                  path = [
                    {
                      src = "${file}";
                      dst = "resources/figures/${path.getBaseName file}";
                    }
                  ];
                  string = [ ];
                });
            ...
            let
              toPlot =
                ...
                type.matchPrimitiveOrPanic data {
                  ...
                  set =
                    let
                      plot =
                        if data.file or null != null then
                          "\"${data.file}\""
                        else if data.eq or null != null then
                          data.eq
                        else
                          error.unimplemented;
                      title = if data.title or null != null then "title \"${data.title}\"" else "notitle";
                      lines = if data.lines or true then "with lines lt" else "";
                      colour = if data.colour or null != null then data.colour else "0x00777777";
                    in
                    "${plot} ${title} ${lines} rgb \"${colour}\"";
                };
              plots =
                if list.isInstanceOf plot.data then
              ...
              genPeaks = list.map (
                ...
                let
                  x' = if set.isInstanceOf x then (x.from + x.till) / 2 else x;
                  x'' = string (number.round x');
                  y' = string y;
                  z' = string (number.round (x' + z));
                  z'' = string (number.round (x' + z - 60));
                in
                if plot.peaksTop or true then
                  ''
                    ...
                    ${
                      if text != null then
                        ''set label "\\tiny (${text})" right rotate by 90 at first ${z''}, graph 0.88''
                      else
                        ""
                    }
                  ''
                  ...
                  ''
                    ...
                    ${
                      if text != null then
                        ''set label "\\tiny (${text})" right rotate by 90 at first ${z''}, graph 0.12''
                      else
                        ""
                    }
                  ''
              ...
              body = string.concatLines (
                [ ]
                ++ (
                  if plot.title or null != null then
                    [ "set title \"{\\\\footnotesize{${plot.title}}}\"" ]
                  else
                    [ "unset title" ]
                )
                ++ (
                  if plot.xLabel or null != null then
                    [ "set xlabel \"{\\\\small{${plot.xLabel}}}\"" ]
                  else
                    [ "unset xlabel" ]
                )
                ++ (
                  if plot.yLabel or null != null then
                    [ "set ylabel \"{\\\\small{${plot.yLabel}}}\"" ]
                  else
                    [ "unset ylabel" ]
                )
              ...
              epsFile = "${dst}.eps";
              texFile = "${dst}.tex";
              gnuplotSrcFile = path.toFile "${hash}.gnuplot" ''
