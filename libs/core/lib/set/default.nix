...
let
  ...
  getKeySource/* :  string -> { string -> T } -> { column: int; file: string; line: int; } */ = intrinsics.unsafeGetAttrPos;
  ...
  # string -> { ... } -> bool
  hasAttribute = intrinsics.hasAttr
    or  (
    name:
    self:
  ...
  pair/* :  n, T @ n:[ string ] -> n:[ T ] -> { T } */ = listOfNames:
    listOfValues:
  ...
  partitionByName = predicate:
    partition (name: _: predicate name);

  partitionByValue = predicate:
