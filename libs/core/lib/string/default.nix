...
let
  ...
  DoNotFollow = type "DoNotFollow" {
    from = value: DoNotFollow.instantiate { inherit value; };
  };
  ...
  inherit (intrinsics) appendContext;

  ascii = list.generate getChar 128;
  ...
  concatWith # :  string -> [ string ] -> string
    =
      intrinsics.concatStringsSep or (
        separator: parts:
        list.fold (result: entry: "${result}${separator}${entry}") (list.head parts) (list.tail parts)
      );
  ...
  concatIndexMappedWith # :  F -> string -> [ T ] -> string
    ...
    #   T: Any:
    =
      convert: separator: parts:
      concatWith separator (list.imap convert parts);
  ...
  concatMappedWith # :  F -> string -> [ T ] -> string
    ...
    =
      convert: separator: parts:
      concatWith separator (list.map convert parts);
  ...
  concatMappedWith' # :  F -> string -> [ T ] -> string
    ...
    =
      convert: separator: parts:
      concatWith separator (set.mapToList convert parts);

  concatWithFinal # :  string -> string -> [ string ] -> string
    =
      separator: final: parts:
      if list.length parts > 1 then
        "${concatWith separator (list.body parts)}${final}${list.foot parts}"
      else
        list.head parts;
  ...
  concatIndexMappedWithFinal # :  F -> string -> string -> [ T ] -> string
    ...
    #   T: Any
    =
      convert: separator: final: parts:
      concatWithFinal separator final (list.imap convert parts);
  ...
  concatMappedWithFinal # :  F -> string -> string -> [ T ] -> string
    ...
    =
      convert: separator: final: parts:
      concatWithFinal separator final (list.map convert parts);
  ...
  concatMappedWithFinal' # :  F -> string -> string -> [ T ] -> string
    ...
    =
      convert: separator: final: parts:
      concatWithFinal separator final (set.mapToList convert parts);

  discardContext = intrinsics.unsafeDiscardStringContext;

  discardOutputDependency = intrinsics.unsafeDiscardOutputDependency;

  escape = replace [ "\"" "\\" "\n" "\r" "\t" char.escape ] [ "\\\"" "\\\\" "\\n" "\\r" "\\t" "\\e" ];

  escapeKey =
    key: if (match "[A-Za-z_][-'0-9A-Za-z_]*" key) != null then key else "\"${escape key}\"";
  ...
  formatAttrPath = concatMapped (
    key: bool.select (integer.isInstanceOf key) "[${integer.toString key}]" ".${key}"
  );
  ...
  formatLambda =
    let
      mapArgument = key: value: "${escapeKey key}${bool.select value "?" ""}";
    ...
    let
      ...
      value' = bool.select (args != { }) "({ ${concatCSV (mapArguments args)} }: ...)" "(_: ...)";
    in
  ...
  formatList =
    ...
    if depth == maxDepth then
      ...
      "[ ${concatMappedWords (value: "(${value})") body} ]"
  ...
  formatPath =
    { depth, maxDepth, ... }: value: if depth == maxDepth then "<path>" else intrinsics.toString value; # "${value}";
  ...
  formatSet =
    ...
    let
      body = set.mapToList format (
        bool.select (value.__public__ or null != null) (set.filterByName value value.__public__) (
          set.remove value [
            "__public__"
            "__type__"
            "__variant__"
      ...
      niceText =
        bool.select (isSimpleSet value) "{ ${list.head body} }"
          "{\n${indent}  ${concatWith "\n${indent}  " body}\n${indent}}";
      niceText' = bool.select isTyped "<${typeName} ${niceText}>" niceText;
      typeName = type.format value;
      value' =
        if debug.Debug.isInstanceOf value then
          "<Debug>"
        else if derivation.isInstanceOf' value then
          "<Derivation ${value.name}>"
        else if library.isInstanceOf value then
          "<Library ${value.getVariant}>"
        else if value._type or null != null then
          "<${value._type}>"
        else if nice && !legacy then
          niceText'
        else if legacy || type.getType value == null then
          "{ ${concatWith " " (set.mapToList format value)} }"
        else if body != [ ] then
  ...
  formatSetValue =
    ...
    in
    bool.select (isSet && !isTyped && isSingleton) "${escapedKey}.${
      formatSetValue env' key' value'
  ...
  formatString =
    ...
    if depth == maxDepth then
      ...
      in
      if list.length lines > 1 then "''${lines'}''" else "\"${value'}\""
    else if display || depth > 0 then
      "\"${value'}\""
    else
      value;
  ...
  formatValue =
    ...
    let
      ...
      format = type.matchPrimitive value {
        bool = formatBool;
        float = formatFloat;
        int = formatInteger;
        lambda = formatLambda;
      ...
      doNotFormat = !showType && maxDepth != null && depth >= maxDepth;

      valueIfUnsuccessful = debug.panic [ "formatValue" "valueIfUnsuccessful" ] {
        text = "Panic occurred while evaluation input value.";
        when = !display || legacy;
      } "<panic>";

      valueIfVeryDeep = debug.panic [ "formatValue" "valueIfVeryDeep" ] {
        text = "Very deep o.O";
        when = veryDeep.panic;
      ...
      value' =
        if doNotFormat then
          ...
          valueIfUnsuccessful
        else if list.find value seen then
          "<recursion>"
        else if depth >= veryDeep.depth then
          valueIfVeryDeep
        else
          format env' value;
    in
    if trace then intrinsics.trace "<$>${formatAttrPath attrPath}" value' else value';
    ...
    =
      let
        ...
        bytes = intrinsics.listToAttrs (
          ...
          ++ [
            {
              name = head (getChar' "0080");
              value = 194;
            }
            {
              name = head (getChar' "00c0");
              value = 195;
            }
          ]
          ++ (list.combine (a: b: {
          ...
          ++ (list.combine (a: b: {
            ...
            value = 128 + 16 * a + b;
          }) (list.range 0 3) (list.range 0 15))
        );
      in
      bytes.${char};
  ...
  getChar # :  int -> char
    = list.get (list.map getChar' (combine hexPairs hexPairs));

  inherit (intrinsics) getContext;

  getFinalChar = self: slice ((length self) - 1) 1 self;

  inherit (intrinsics) hasContext;

  hash # :  string -> string -> string
    = intrinsics.hashString;

  ifOrEmpty = condition: text: if condition then text else "";
  ...
  length # :  string -> int
    =
      intrinsics.stringLength or (
        text:
        let
          rest = slice 1 9223372036854775807 text;
        in
        if text == "" then 0 else (length rest) + 1
      );
  ...
  inherit (intrinsics) match;

  orNull = value: isInstanceOf value || value == null;

  repeat # :  string -> int -> string
    =
      text: multiplier: concat (list.generate (_: text) multiplier);

  replace # :  [ string ] -> [ string ] -> string -> string
    = intrinsics.replaceStrings; # should be possible to construct, but ahhh

  replace' = substitutions: replace (set.names substitutions) (set.values substitutions);

  slice # :  int -> int -> string -> string
    = intrinsics.substring;

  inherit (intrinsics) split;

  splitAt # :  string -> string -> [ string ]
    =
      regex: text: list.filter isInstanceOf (split regex text);

  splitAt' # :  string -> string -> [ string ]
    =
      regex: text: list.filter (line: isInstanceOf line && line != "") (split regex text);

  splitLines # :  string -> [ string ]
    = splitAt "\n";

  splitSpaces = splitAt "([[:space:]]+)";
  ...
  toCharacters # :  string -> [ asciiChar ]
    =
      text: list.generate (index: slice index 1 text) (length text);
  ...
  toLowerCase # :  string -> string
    =
      ...
      in
      text: list.fold (text: char: "${text}${caseMap.${char} or char}") "" (toUTF8characters text);

  toString # :  T -> string
    =
      intrinsics.toString or (format {
        display = false;
        hex = false;
        legacy = true;
        maxDepth = null;
  ...
  toUpperCase # :  string -> string
    =
      ...
      in
      text: concatMapped (char: "${caseMap.${char} or char}") (toUTF8characters text);
        ...
        this =
          list.fold
            (
              { result, text }:
              character:
              if character <= char.delete then
                {
                  text = "";
                  result = result ++ (if text != "" then [ text ] else [ ]) ++ [ character ];
                }
              else
                {
                  # Does not validate!
            ...
            (toCharacters text);
      in
      this.result ++ (if this.text != "" then [ this.text ] else [ ]);
...
type "string" {
  ...
  inherit
    ...
    concatMapped'
    concatComma
    concatIndexMappedComma
    concatMappedComma
    concatMappedComma'
    concatCRLF
    concatIndexMappedCRLF
    concatMappedCRLF
    concatMappedCRLF'
    concatCSV
    concatIndexMappedCSV
    concatMappedCSV
    concatMappedCSV'
    concatLines
    concatIndexMappedLines
    concatMappedLines
    concatMappedLines'
    concatWith
    concatIndexMappedWith
    concatMappedWith
    concatMappedWith'
    concatWithFinal
    concatIndexMappedWithFinal
    concatMappedWithFinal
    concatMappedWithFinal'
    concatWords
    concatIndexMappedWords
    concatMappedWords
    concatMappedWords'
    discardContext
