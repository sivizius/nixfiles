...
let
  ...
  checkSecrets = set.mapToListConcatted (
    name: value:
    let
      expect =
        entry:
        if value.${entry} != null then
          [ ]
        else
      ...
      extra =
        ...
        if value.${entry} != null then
          ...
          [ ];
    in
    if value.type or null != null then
      {
      ...
      [ "${name} is null, missing generateToken?" ]
    else if list.isInstanceOf value then
      [ "${name} is a list, missing generateEnvFile?" ]
    else
      [ "${name} is invalid: ${type.getPrimitive value}" ]
  );
  ...
  decryptFiles =
    ...
    set.mapToListConcatted (
      ...
      in
      if type == "decrypt" then
        (gpg.decrypt args encryptedFile fileName) ++ (setPermissions { inherit fileName group owner; })
      else
        [ ]
    );
  ...
  generateEnvFiles =
    ...
    set.mapToListConcatted (
      ...
      if type == "generateEnvFile" then
        let
          fileName = "${vaultBasePath}/${name}";
        in
        [ ''echo -n "" > "${fileName}"'' ]
        ++ (list.map (
          variable: ''echo "_${variable}=$(${utils}/cat "${vaultBasePath}/${variable}")" >> "${fileName}"''
        ) variables)
  ...
  generateTokens =
    ...
    set.mapToListConcatted (
      ...
      if type == "generateToken" then
        ...
        in
        [ generate ] ++ (setPermissions { inherit fileName group owner; })
      else
        [ ]
    );
...
let
  ...
  daemon = pkgs.writeShellScript "initVault.sh" (
    string.concatLines (
      [
        ''set -e''
