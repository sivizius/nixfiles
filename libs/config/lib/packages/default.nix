...
let
  ...
  checkPackage =
    ...
    debug.info "checkPackage" "Check Package `${name}`…" (
      if debug.Debug.isInstanceOf package then
        abort "${package}"
      else if success && derivation.isInstanceOf' value then
        value
  ...
  defaultAllowPredicate =
    predicateName: allowAll: allowedPackages:
    {
      name ? null,
  ...
  extractPackageName = name: if name != null then list.head (string.split "-" name) else null;
  ...
  fixConfig =
    {
      ...
      allowNonSourcePredicate ? (
        defaultAllowPredicate "non-source" allowNonSource allowedNonSourcePackages
      ),
      allowUnfree ? false,
  ...
  fromNixpkgs =
    ...
    target.System.mapStdenv (
      ...
      in
      if registry != null then legacyPackages'.${registry} else legacyPackages'
    );
    ...
    let
      loadPackage =
        ...
        type.matchPrimitiveOrPanic package {
          ...
          set = debug.trace [ "load" "loadPackage" ] {
            text = "Package:";
            data = package;
            nice = false;
          } [ package ];
      ...
      loadPackages = source: list.concatIMap (index: loadPackage (source index));

      packageList = loadPackage (context fileName) fileName;

      packages = list.fold (
        result:
        { name, ... }@package:
        if !(set.hasAttribute name result) then
          result
          // {
            ${name} = package;
          }
        else if result.${name} == package then
          result
        else
