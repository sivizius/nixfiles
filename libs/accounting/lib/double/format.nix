...
let
  ...
  concatLines' = function: lines: string.concatLines (list.filter (value: value != null) (list.map function lines));

  lineLength = 111;

  padNameValue/* :  string -> string */ = name:
    value:
    currency:
    extra:
    let
  ...
  formatBalance/* :  Self -> Date -> string */ = dateTime:
    self:
    formatBalance' (balance dateTime self);

  formatBalance'/* :  Self -> string */ = { name, currency, dateTime, assets, liabilities, outcome, journal, events ? { }, ... } @ self:
    let
      formatBalanceTitle = events.formatBalanceTitle
        or  (
        { name, dateTime, ... }:
      ...
      formatAccountNames = events.formatAccountNames
        or  (
        { ... }:
        {
      ...
      formatBalanceNames = events.formatBalanceNames
        or  (
        { ... }:
        {
          credit = "Credit";
      ...
      listSections/* :  [ Transaction ] -> bool -> int -> T -> string */
        ...
        let
          ...
          formatAccount/* :  int -> { name, credit, debit, total, ... } -> string */ = { name, credit, debit, total, ... }:
            let
              listTransactions/* :  [ Transaction ] -> [ { amount: Amount, reference: int, ... } ] -> string */ = list.concatMap
                (
                  { amount, reference, ... }:
                  let
  ...
  formatOutcome'/* :  Self -> { from: D, till: D } -> string */
    ...
    let
      formatOutcomeTitle = events.formatOutcomeTitle
        or  (
        { name, from, till, ... }:
