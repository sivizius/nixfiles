...
let
  ...
  addColumns = { byte0, byte1, byte2, byte3, ... } @ self:
  ...
  rotateColumn = { byte0, byte1, byte2, byte3, ... }:
    {
      byte0 = byte1;
      byte1 = byte2;
      byte2 = byte3;
  ...
  substituteColumn = { byte0, byte1, byte2, byte3, ... }:
    {
    ...
    type.matchPrimitiveOrPanic buffer
      {
        ...
        string = packDWordBuffer (list.map string.getByte (string.toCharacters buffer));
      };

  unsubstituteByte = list.get (path.import ./isbox.nix);

  unsubstituteColumn = { byte0, byte1, byte2, byte3, ... }:
    {
  ...
  unshiftRows4andUnsubstitute = state:
    let
