...
let
          ...
          let
            ...
            symmetric =
              if peak.sym == null then
                ""
              else if peak.sym then
                "\\textsubscript{s}"
              else
                "\\textsubscript{as}";
          in
          if peak.assignment == null then
            null
          else if peak.type == null then
            "${assignment}"
          else
            "${symmetric}${type}(${assignment})"
        )
      ]
    );
  ...
  getXpos =
    peak:
    if set.isInstanceOf peak.x then
      "${string (number.round peak.x.from)}–${string (number.round peak.x.till)}"
    else
      (string (number.round peak.x));
  ...
  reportSpectrum =
    ...
    let
      ...
      signals' = list.sort (
        ...
        let
          first' =
            if list.isInstanceOf first.x then
              list.head first.x
            else if set.isInstanceOf first.x then
              first.x.from
            else
              first.x;
          second' =
            if list.isInstanceOf second.x then
              list.head second.x
            else if set.isInstanceOf second.x then
              second.x.from
            else
              second.x;
        in
        debug.info "compare ir-signals" {
          data = {
            first = first.x;
            second = second.x;
          };
      ...
      spectrum' = list.map (peak: "${getXpos peak} (${getDetails spectrum.min peak})") signals';
      note = if spectrum.note or null != null then "${spectrum.note}, " else "";
    in
    if spectrum' != [ ] then
      LaTeX (
        [
  ...
  genPeaks = list.map (
    peak:
    let
      x' = if set.isInstanceOf peak.x then (peak.x.from + peak.x.till) / 2 else peak.x;
    ...
    if plot != null then
      let
        fileName =
          ...
          in
          if matching != null then list.head matching else plot';
        ...
        epsFile = "${dst}.eps";
        texFile = "${dst}.tex";
        gnuplotSourceFile = path.toFile "${hash}.gnuplot" ''
        ...
        note' = if note != null then " (${note})" else "";
      in
      Heading' "${acronyms.infrared.short}-Spektrum~${substance.NameID}${note'}"
        [
          (LaTeX (
            (if label != null then [ "\\labelAppendix{ir:${label}}%" ] else [ ])
            ++ (
              if true then
                [
                  "\\vspace{-1\\normalbaselineskip}"
        ...
        ]
        {
          ...
          dependencies = [
            {
              src = gnuplotSourceFile;
              dst = gnuplotDestinationFile;
            }
            { inherit dst src; }
          ];
        }
  ...
  genAppendix' =
    ...
    let
      ir = product.ir or product.substance.ir or null;
      genAppendix' = genAppendix resources product;
    in
    if ir != null && !failure then
      if list.isInstanceOf ir then list.map genAppendix' ir else [ (genAppendix' ir) ]
    else
      [ ];
  generateAppendix =
    syntheses:
        ...
        PhantomHeading (
          list.concatMap (
            ...
            if synthesis ? product then
              ...
              in
              if list.isInstanceOf product then
                list.map (genAppendix' resources) product
              else
                [ (genAppendix' resources product) ]
            else
              [ ]
          ) (if list.isInstanceOf syntheses' then syntheses' else syntheses'.list)
        )
      ))
    ];
  ...
  report =
    ...
    in
    if list.isInstanceOf spectra then
      list.filter (x: x != null) (list.map reportSpectrum' spectra)
    else
      [ (reportSpectrum' spectra) ];

  Spectrum = plot: config: signals: {
  ...
  Signal = x: y: z: type: sym: assignment: {
    x = type.matchPrimitiveOrDefault x {
      set = { inherit (x) from till; };
      list = {
        from = list.get x 0;
        till = list.get x 1;
      };
