...
let
  ...
  acvHeaderSocialSep = "\\unskip\\enspace\\cleaders\\copy\\acvHeaderSocialSepBox\\hskip\\wd\\acvHeaderSocialSepBox\\enspace\\ignorespaces";

  formatEmail = address: urls.formatEmailTeXboxed address "${icons.email-bulk}\\,${address}";
  ...
  formatGit =
    ...
    let
      icon =
        ...
        .${domain} or null;
      icon' = if isGitea then icons.gitea else icons.git;
    in
    if icon != null then
  ...
  formatHomePage =
    homepage: urls.formatHttpsTeXboxed homepage "${icons.globe}\\,\\texttt{${homepage}}";

  formatLinkedIn =
    { id, name }: urls.formatHttpsTeXboxed "www.linkedin.com/in/${id}" "${icons.linkedin}\\,${id}";
  ...
  formatMedium =
    { id, name }: urls.formatHttpsTeXboxed "medium.com/@${id}" "${icons.medium}\\,${name}";

  formatOrcid = { id, name }: urls.formatHttpsTeXboxed "orcid.org/${id}" "${icons.orcid}\\,${id}";

  formatPGP =
    { fingerprint, url }:
    urls.formatHttpsTeXboxed url "${icons.gnupg}\\,\\texttt{${url} (\\texttt{${fingerprint}})}";

  formatPhoneNumber = icon: number: "${icon}\\,${phonenumbers.formatTeX number}";

  formatReddit =
    name: urls.formatHttpsTeXboxed "www.reddit.com/user/${name}" "${icons.reddit}\\,u/${name}";
  ...
  formatTwitter =
    name: urls.formatHttpsTeXboxed "www.twitter.com/${name}" "${icons.twitter}\\,${name}";
  ...
  formatXing =
    { id, name }: urls.formatHttpsTeXboxed "www.xing.com/profile/${id}" "${icons.xing}\\,${name}";
