...
let
  ...
  buildDirectory =
    ...
    let
      ...
      link = set.mapToList (dst: src: ''$coreutils/ln -s "${src}" "$out/${dst}"'') body;
      mkdir = set.values (
        list.fold (
          result: fileName:
          let
  ...
  writeConfigFile = this: buildTextFile (this // { permissions = "444"; });

  writeScriptFile = this: buildTextFile (this // { permissions = "555"; });
