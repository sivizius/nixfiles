...
let
  ...
  concatWith/* :  string -> [ string ] -> string */ = intrinsics.concatStringsSep
    or  (
    separator:
    parts:
    list.fold
  ...
  concatWithFinal/* :  string -> string -> [ string ] -> string */ = separator:
    final:
    parts:
  ...
  formatValue = { attrPath, depth, display, legacy, maxDepth, seen, showType, trace, ... } @ env:
    input:
    let
      env' = env
        // {
  ...
  getByte/* :  string -> int */ = text: getByte' (slice 0 1 text);

  getByte'/* :  string -> int */ =
    let
      get/* :  string -> int -> string */ = text: index: slice index 1 text;

      head/* :  string -> char */ = text: get text 0;
  ...
  getChar'/* :  string -> char */ = index: expression.fromJSON "\"\\u${index}\"";

  getChar/* :  int -> char */ = list.get (list.map getChar' (combine hexPairs hexPairs));
  ...
  hash/* :  string -> string -> string */ = intrinsics.hashString;
  ...
  lengthUTF8/* :  string -> int */ = text: list.length (toUTF8characters text);
  ...
  repeat/* :  string -> int -> string */ = text:
    multiplier:
    concat (list.generate (_: text) multiplier);

  replace/* :  [ string ] -> [ string ] -> string -> string */ = intrinsics.replaceStrings; /* should be possible to construct, but ahhh */
  ...
  slice/* :  int -> int -> string -> string */ = intrinsics.substring;

  inherit (intrinsics) split;

  splitAt/* :  string -> string -> [ string ] */ = regex:
    text:
    list.filter
      isInstanceOf
      (split regex text);
  ...
  splitLines/* :  string -> [ string ] */ = splitAt "\n";

  splitSpaces = splitAt "([[:space:]]+)";

  splitTabs/* :  string -> [ string ] */ = splitAt "\t";

  toBytes/* :  string -> [ u8 ] */ = text: list.map getByte' (toCharacters text);

  toCharacters/* :  string -> [ asciiChar ] */ = text:
    list.generate (index: slice index 1 text) (length text);

  toPath/* :  string -> path */ = path: "./${path}";
  ...
  toString/* :  T -> string */ = intrinsics.toString
    or  (
    format
      {
        display = false;
  ...
  toUTF8characters/* :  string -> [ utf8char ] */ = text:
    let
      this = list.fold
        (
          { result, text }:
