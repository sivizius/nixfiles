  ...
  registerAccount =
    ...
    }:
    {
      ...
      neomutt = {
        ...
        extraConfig =
          let
            extraConfig = string.concatLines (
              [ ] ++ (list.ifOrEmpty (gpg.key or null != null) ''set pgp_sign_as = "${gpg.key}"'')
            );
            signatureFile = bool.select (
              signature != null
            ) (path.toFile "signature.txt" signature) "\"fortune |\"";
          in
          ''
            unmailboxes *
      ...
      passwordCommand =
        let
          passwordCommand = store.write.shellScript "passwordCommand" ''
            if [ $# -gt 1 ]
            then
