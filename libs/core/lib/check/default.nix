...
let
  TestCase = type "TestCase" {
    from = { success, value }@result: TestCase.instantiate result;
  };

  TestResult = type "TestResult" {
    from = variant: { success, value }@result: TestResult.instantiateAs variant result;
  };
  ...
  checkValue =
    ...
    type.matchPrimitiveOrDefault result.value {
      list = checkList result.value;
      set = if !(string.DoNotFollow.isInstanceOf result.value) then checkSet result.value else result;
    } (TestResult "Value" result);

  format =
    system: fullName:
    { success, value, ... }@this:
    if !(TestResult.isInstanceOf this) then
      string.concatLines (
        set.mapToList (
          name: testCases:
          let
            fullName' = if fullName != null then "${fullName} → ${name}" else name;
          in
      ...
      let
        ...
        fullName' = if fullName != null then " ${fullName}" else "";
        value' = string.from {
          legacy = false;
          display = true;
          maxDepth = null;
        ...
        verdict = if success then "${foreground.green}[passed]" else "${foreground.red}[failed]";
        formatLine = line: "echo -e \"${verdict}${fullName'} @ ${system}${line}${ansi.reset}\"";
      in
  ...
  testsToChecks =
    ...
    let
      ...
      builder = path.toFile "builder.sh" ''
        #!/usr/bin/env sh
...
in
{
  ...
  __functor =
    ...
    tests: arguments:
    if false then
      target.System.mapAll (buildSystem: {
