...
let
  ...
  extensionOf =
    ...
    in
    if name != null then list.get name ((list.length name) - 1) else null;
  ...
  formatSpectrum # :  Spectrum -> Chunk
    =
      ...
      let
        solvent' = if solvent != null then solvent else "${acronyms.CDCl3.short}";
        title' =
          if title != null then
        ...
        label' = "nmr:${label}";
      in
      if (extensionOf file == "pdf") then
        Heading' "${title'}"
          [
            (LaTeX (
              (if label != null then [ "\\labelAppendix{nmr:${label}}%" ] else [ ])
              ++ (
                if true then
                  [
                    "\\vspace{-1\\normalbaselineskip}"
  ...
  formatSpectrumName # Spectrum -> string
    =
      ...
      let
        nucleus' =
          if nucleus == "1Hx13C" then
            "${formatNMRnucleus "1H"}-${formatNMRnucleus "13C"}"
          else
            formatNMRnucleus nucleus;
        dept # :  integer -> string
          =
            degree: "${formatNMRnucleus nucleus}-${acronyms.dept.short}-${string degree}";
      in
      {
        ...
        spectra =
          list.concatMap
                ...
                list.map
                  ...
                  )
                  {
                    "1H" = [
                      {
                        kind = "self";
                        files = files.self or files."1H" or null;
                      }
                      {
                        kind = "cosy";
                        files = files.cosy or null;
                      }
                      {
                        kind = "tocsy";
                        files = files.tocsy or null;
                      }
                      {
                        kind = "noesy";
                        files = files.noesy or null;
                      }
                      {
                        kind = "roesy";
                        files = files.roesy or null;
                      }
                    ];
                    "13C" = [
                      {
                        kind = "dc";
                        files = files.dc or files."13C{1H}" or null;
                      }
                      {
                        kind = "self";
                        files = files.self or files."13C" or null;
                      }
                      {
                        kind = "apt";
                        files = files.apt or null;
                      }
                      {
                        kind = "dept";
                        files = files.dept or null;
                      }
                      {
                        kind = "dept90";
                        files = files.dept90 or null;
                      }
                      {
                        kind = "dept135";
                        files = files.dept135 or null;
                      }
                    ];
                    "15N" = [
                      {
                        kind = "dc";
                        files = files.dc or files."15N{1H}" or null;
                      }
                      {
                        kind = "self";
                        files = files.self or files."15N" or null;
                      }
                    ];
                    "19F" = [
                      {
                        kind = "dc";
                        files = files.dc or files."19F{1H}" or null;
                      }
                      {
                        kind = "self";
                        files = files.self or files."19F" or null;
                      }
                    ];
                    "31P" = [
                      {
                        kind = "dc";
                        files = files.dc or files."31P{1H}" or null;
                      }
                      {
                        kind = "self";
                        files = files.self or files."31P" or null;
                      }
                    ];
                    "1Hx13C" = [
                      {
                        kind = "hsqc";
                        files = files.hsqc or null;
                      }
                      {
                        kind = "hmbc";
                        files = files.hmbc or null;
                      }
                    ];
                  }
                  .${nucleus}
            ...
            [
              ...
              {
                nucleus = "13C";
                data = nmr'."13C" or null;
              }
              {
                nucleus = "15N";
                data = nmr'."15N" or null;
              }
              {
                nucleus = "19F";
                data = nmr'."19F" or null;
              }
              {
                nucleus = "31P";
                data = nmr'."31P" or null;
              }
              {
                nucleus = "1Hx13C";
                data = nmr'."1Hx13C" or null;
              }
            ];
      in
      if nmr' != null && !failure then list.concatMap (genAppendixForSpectrum resources) spectra else [ ];
  ...
  genAppendixForSpectrum # :  Spectrum -> [ Chunk ] | !
    =
      ...
      let
        ...
        formatSpectrum' =
          ...
          formatSpectrum resources {
            ...
            sample = if sample != null then " (${string sample})" else "";
          };
      in
      type.matchPrimitiveOrPanic files {
              ...
              // {
                ...
                list = state.list ++ [
                  (type.matchPrimitiveOrPanic file {
                    lambda = debug.panic "genAppendix" "2???";
                    path = formatSpectrum' null file (
                      spectrum.label or "${substance.name or name}_${nucleus}_${string state.counter}"
                    );
                    set = formatSpectrum' (file.identifier or null) file.file (
        ...
        path =
          let
            kind' = if kind == "self" then nucleus else "${nucleus}${kind}";
            spectrum' = formatSpectrum' null files (spectrum.label or "${substance.name or name}_${kind'}");
          in
          [ spectrum' ];
        set =
          let
            identifier = if files.identifier or null != null then "_${string files.identifier}" else "";
          in
          [
            (formatSpectrum' (files.identifier or null) files.file (
  ...
  genDependencies # :  Product -> Dependencies
    =
      ...
      let
        nmr' = substance.nmr or nmr;
        files = list.concatMap (spectrum: if spectrum != null then set.values spectrum.files else [ ]) [
          (nmr'."1H" or null)
          (nmr'."13C" or null)
          (nmr'."15N" or null)
          (nmr'."19F" or null)
        ...
        getFileName =
          fileName: "resources/appendix/nmr/${substance.name or name}/${path.getBaseName fileName}";
      in
      if nmr' != null then
        list.concatMap (
          file:
          type.matchPrimitiveOrDefault file {
            ...
            path = [
              {
                src = file;
                dst = getFileName file;
              }
            ];
            set = [
              {
                src = file.file;
                dst = getFileName file.file;
              }
            ];
          }
        ) files'
    ...
    =
      ...
      list.map (
        ...
        let
          other' =
            if other != null then
              if list.isInstanceOf other then
          ...
          prepareRange =
            precision: range:
            if list.isInstanceOf range then
              let
            ...
            else if string.isInstanceOf range then
              range
          ...
          mapCouplings = set.mapToList (
            coupling: list:
            let
              result = string.match "([0-9]+)([A-Za-z]+)" coupling;
              bonds = if result != null then "\\textsuperscript{${list.get result 0}}" else "";
              nuclei = if result != null then list.get result 1 else coupling;
              values = physical.formatValue {
                value = list;
                precision = 1;
              } "hertz";
            in
          ...
          element =
            ...
            in
            if nucleus' != null then list.head nucleus' else nucleus;
            ...
            let
              charge' =
                if charge == 0 then
                  ...
                  "\\textsuperscript{\\minus}"
                else if charge < 0 then
                  "\\textsuperscript{\\minus${string (0 - charge)}}"
                else
                  "\\textsuperscript{+${string charge}}";
            in
            if protons == 0 then
              let
                saturated =
  ...
  reportSpectra # :  NMRdata -> [ LaTeX ] | !
    =
      ...
      let
        ...
        solvent = if nmrData.solvent != null then nmrData.solvent else "${acronyms.CDCl3.short}";

        hyperlink = if nmrData.label or null != null then "\\hyperlink{appendix:${nmrData.label}}" else "";
        suffix =
          let
            methods = list.filter ({ method, from }: from) [
              {
                method = "apt";
                from = method.apt or false;
              }
              {
                method = "dept";
                from = method.dept or false;
              }
              {
                method = "dept45";
                from = method.dept45 or false;
              }
              {
                method = "dept90";
                from = method.dept90 or false;
              }
              {
                method = "dept135";
                from = method.dept135 or false;
              }
              # { method = "cosy";    from = method.cosy    or false; }
              # { method = "hmbc";    from = method.hmbc    or false; }
              {
                method = "hsqc";
                from = method.hsqc or false;
              }
        ...
        nucleus =
          {
            "1H" = if files ? self then "1H" else null;
            "13C" =
              if files ? self then
                "13C"
              else if files ? dc then
                "13C{1H}"
              else
                null;
            "15N" =
          ...
          .${kind};
        sortSignals = list.sort (
          first: second:
          let
            first' = if list.isInstanceOf first.range then list.head first.range else first.range;
            second' = if list.isInstanceOf second.range then list.head second.range else second.range;
          in
          first' > second'
        );
      in
      if nucleus != null && signals != [ ] then
        [
          (LaTeX (
            [
...
in
{
  ...
  Signal' =
    ...
    Signal range { inherit integral multiplicity; };
  SignalAP =
    range: protons: integral:
    Signal range { inherit protons integral; };
  SignalDC = range: integral: Signal range { inherit integral; };
  SignalJ =
    range: multiplicity: couplings: integral:
    Signal range { inherit couplings integral multiplicity; };
  ...
  generateAppendix =
    ...
    let
      syntheses' =
        if path.isInstanceOf syntheses then
          import syntheses (libs // { inherit (libs) chemistry; }) document
        else
          syntheses;
      syntheses'' = if list.isInstanceOf syntheses' then syntheses' else syntheses'.list;
      products = list.concatMap (
        # Synthesis -> [ string ]
        synthesis:
        if synthesis ? product then
          let
            product =
              if lambda.isInstanceOf synthesis.product then synthesis.product { } else synthesis.product;
            product' = product // {
              name = product.name or null;
              substance = product.substance or synthesis.substance or { };
            };
          in
          if list.isInstanceOf product then product else [ product' ]
        else
          [ ]
      ) syntheses'';
    in
    LaTeX'
      (
        ...
        ++ [
          (PhantomHeading (PhantomHeading (list.concatMap (genAppendix resources) products)))
          "\\restoregeometry%"
          null
        ]
      )
  ...
  report =
    resources: nmrData:
    list.concatMap
      ({ nmrData, ... }@spectra: if nmrData != null then reportSpectra resources spectra else [ ])
      [
        {
          kind = "1H";
          nmrData = nmrData."1H" or null;
        }
        {
          kind = "13C";
          nmrData = nmrData."13C" or null;
        }
        {
          kind = "15N";
          nmrData = nmrData."15N" or null;
        }
        {
          kind = "19F";
          nmrData = nmrData."19F" or null;
        }
        {
          kind = "31P";
          nmrData = nmrData."31P" or null;
        }
      ];
}
