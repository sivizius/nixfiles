...
let
  ...
  checkTransactionAccounts/* :  { Account } -> { Account } -> Transaction -> [ string ] */ = allAccounts:
    transactionAccounts:
    transaction:
    let
  ...
  getUnknownAccounts/* :  { Accounts } -> [ string ] -> [ string ] */ = allAccounts:
    checkAccounts:
    list.filter
  ...
  lookUp =
    let
      ...
      match = { description ? null, email ? null, uid ? null, ... } @ client:
        debug.info [ "Book" "lookUp" "match" ]
          ...
          (
            ...
            let
              regexMatch = (uid != null && regex.uid          or null != null && ((string.match regex.uid uid) != null))
  ...
  toHTML =
    let
      ...
      render = { body, accounts, journal, ... } @ subbook:
        HTML
          [
            (
              HTML.Block' "head"
