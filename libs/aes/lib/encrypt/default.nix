...
let
  ...
  slowRoundEncryption =
    let
      mixColumns = list.map (
        ...
        in
        {
          byte0 = list.fold integer.xor byte0' [
            byte3
            byte2
            byte1'
            byte1
          ];
          byte1 = list.fold integer.xor byte1' [
            byte0
            byte3
            byte2'
            byte2
          ];
          byte2 = list.fold integer.xor byte2' [
            byte1
            byte0
            byte3'
            byte3
          ];
          byte3 = list.fold integer.xor byte3' [
            byte2
            byte1
            byte0'
            byte0
    ...
    in
    state: mixColumns (substituteAndShiftRows4 state);
  ...
  fastRoundEncryption =
    ...
    let
      ...
      column0' = unpackDWord (
        integer.xor (integer.xor (subTE0 column0.byte0) (subTE1 column1.byte1)) (
          integer.xor (subTE2 column2.byte2) (subTE3 column3.byte3)
        )
      );
      column1' = unpackDWord (
        integer.xor (integer.xor (subTE0 column1.byte0) (subTE1 column2.byte1)) (
          integer.xor (subTE2 column3.byte2) (subTE3 column0.byte3)
        )
      );
      column2' = unpackDWord (
        integer.xor (integer.xor (subTE0 column2.byte0) (subTE1 column3.byte1)) (
          integer.xor (subTE2 column0.byte2) (subTE3 column1.byte3)
        )
      );
      column3' = unpackDWord (
        integer.xor (integer.xor (subTE0 column3.byte0) (subTE1 column0.byte1)) (
  ...
  encrypt # :  AESkey -> [ u8 ]
    =
      ...
      let
        ...
        state' = list.fold applyRoundEncryption' state roundKeys';
      in
      addRoundKey (substituteAndShiftRows4 state') lastRoundKey;
in
{
  __functor = { ... }: encrypt;
