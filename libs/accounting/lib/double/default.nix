...
let
  ...
  addReferences/* :  [ { ... } ] -> [ { reference: int, ... } ] */ = list.imap
    (reference: this: this // { inherit reference; });

  balance/* :  Self -> ~DateTime-> Balance | ! */ = balanceTime:
    self:
    let
      self' = from self;
  ...
  balance'/* :  Self -> Balance | ! */ = { accounts, assets, liabilities, outcome, journal, ... } @ self:
    let

      accounts'/* :  { Account } */ = list.fold
        (
          accounts:
  ...
  balanceToTransaction/* :  Balance -> Transaction */ = { accounts, dateTime, events ? { }, ... } @ self:
    let
    ...
    // {
      inherit dateTime;
      debit = set.filterValue
        ({ total, ... }: total > 0)
        accounts;
      credit = set.mapValues
        (total: -total)
        (
          set.filterValue
            ({ total, ... }: total < 0)
  ...
  from/* :  { name, assets: T, liabilities: T, outcome: T, journal: [ ~Transaction ] } -> Self */
    ...
    let
      ...
      flatAccounts/* :  [ { id, name, ... } | { title, level, ... } ] -> { Account } */ = accounts:
        list.mapValuesToSet
  ...
  outcome/* :  Self -> { from: Date, till: ~DateTime } -> { } | ! */ = { from, till }:
    self:
    let
      splitTransaction/* :  [ Transaction ] -> ~DateTime -> ~DateTime -> { current: [ Transaction ], before: [ Transaction ] } */ = journal:
        let
