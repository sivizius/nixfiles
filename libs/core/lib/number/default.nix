...
let
  assertNumber/* :  int | float -> int | float | ! */ = value:
    matchNumber value
      {
        float = value;
        int = value;
  ...
  add/* :  int | float -> int | float -> int | float */ = intrinsics.add or (a: b: (assertNumber a) + (assertNumber b));

  and/* :  int -> int -> int */ = intrinsics.bitAnd;

  inherit (intrinsics) ceil;/* :  int | float -> int */

  div/* :  int | float -> int | float -> int | float */ = intrinsics.div or (a: b: (assertNumber a) / (assertNumber b));

  inherit (intrinsics) floor;/* :  int | float -> int */

  isInstanceOf = value:
    integer.isInstanceOf value
    || float.isInstanceOf value;

  lessThan/* :  int | float -> int | float -> int | float */ = intrinsics.lessThan or (a: b: (assertNumber a) < (assertNumber b));

  moreThan/* :  int | float -> int | float -> int | float */ = a: b: (assertNumber a) > (assertNumber b);

  mul/* :  int | float -> int | float -> int | float */ = intrinsics.mul or (a: b: (assertNumber a) * (assertNumber b));

  neg/* :  int | float -> int | float */ = value: (0 - (assertNumber value));

  or/* :  int -> int -> int */ = intrinsics.bitOr;
  ...
  pow/* :  int -> int | float -> int | float */ =
    let
      pow/* :  int -> int | float -> int | float */ = base:
        exp:
        list.fold
          (y: x: x * y)
          1.0
  ...
  round'/* :  float -> int */ = value: floor (value + 0.5);

  sub/* :  int | float -> int | float -> int | float */ = intrinsics.sub or (a: b: (assertNumber a) - (assertNumber b));

  sum/* :  [ int | float ] -> int | float */ = list.fold (result: value: result + value) 0;

  toFloat/* :  int | float | string -> float */ = value:
  ...
  toInteger/* :  int | float | string -> int */ = value:
    type.matchPrimitiveOrDefault value
      {
        float = round' value;
        int = value;
  ...
  toInteger'/* :  string -> int | null */ = value:
    let
  ...
  toStringWithMaximumPrecision/* :  int | float -> int -> string */ = value:
    precision:
    if precision > 0
    then
      list.foldReversed
  ...
  toStringWithPrecision/* :  int | float -> int -> string */ = value:
    precision:
    let
      precision' = type.matchPrimitiveOrDefault precision
        {
          int = precision;
      ...
      valueWithPrecision = debug.info "toStringWithPrecision" { text = "value"; data = value; }
        debug.info "toStringWithPrecision"
        { text = "precision"; data = [ precision precision' ]; }
        (
          if precision' == 0
          then
            "${string (round value)}"
  ...
  toSignificantString/* :  int | float -> string */ = value:
  ...
  xor/* :  int -> int -> int */ = intrinsics.bitXor;
in
type "number"
{
