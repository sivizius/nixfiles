...
let
  ...
  genAppendix' =
    ...
    let
      ms = product.ms or product.substance.ms or null;
    in
    if ms != null && !failure then genAppendix resources product ms else [ ];
        ...
        PhantomHeading (
          list.concatMap (
            ...
            if synthesis ? product then
              let
                product = if type.isLambda synthesis.product then synthesis.product { } else synthesis.product;
              in
              if type.isList product then
                list.map (genAppendix' resources) product
              else
                [ (genAppendix' resources product) ]
            else
              [ ]
          ) (if type.isList syntheses' then syntheses' else syntheses'.list)
        )
      ))
    ];
  ...
  mapIons = list.map (
    ...
    let
      ...
      mass = if calculated != null then calculated else calculateMassOfFormula formula';
      ...
      [
        (LaTeX (
          let
            ...
            highRes' =
              if highRes then acronyms.highResolutionMassSpectrometry.short else acronyms.massSpectrometry.short;
          in
...
in
{
  ...
  report = resources: data: if type.isSet data && data != { } then report resources data else [ ];
}
