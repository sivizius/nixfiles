...
let
  ExecutableFormat/* :  Enum { elf, macho, pe, unknown, wasm } */ = type.enum "ExecutableFormat"
    {
      elf = 1;
      macho = 2;
      pe = 3;
  ...
  Kernel# struct { executableFormat: ExecutableFormat, family: Family }
    = type "Kernel"
    {
      ...
      isDarwin/* :  System -> bool */ = { kernel, ... } @ self:
        kernel == all.darwin;

      isLinux/* :  System -> bool */ = { kernel, ... } @ self:
        kernel == all.linux;

      from/* :  type -> (string | Kernel)? -> Kernel */ = kernel:
  ...
  all/* :  { string -> Kernel } */ =
    let
      new/* :  ExecutableFormat -> Family -> Kernel */ = executableFormat:
        family:
        Kernel.instantiate
          {
