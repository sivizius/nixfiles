...
let
  ...
  ProfileConfiguration = Configuration "Profile" {
    call =
      { environment, host, ... }:
      {
        arguments,
      ...
      wrap {
        configuration = configuration (
          environment'
          // {
            config = debug.warn "ProfileConfiguration" "Argument `config` is deprecated!" environment.config;
          }
        );
      };
  };
  ...
  collect =
    ...
    if legacy then
      ...
      debug.debug "collect" {
        ...
        data = this;
        nice = true;
      } (list.map ProfileConfiguration (configurations.collect args))
  ...
  load =
    source: environment:
    configurations.load source environment (constructors // services.constructors) Profile;
...
// {
  ...
  importLegacy =
    ...
    modules:
    mapLegacy (
      list.mapNamesToSet (name: {
        configuration = path.import "${nixpkgs}/nixos/modules/profiles/${name}.nix";
        source = context "nixpkgs/nixos/modules/profiles" {
