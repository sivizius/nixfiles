...
let
  ...
  pinentry = if self.pinentry != null then "pinentry-program ${self.pinentry}/bin/pinentry" else "";

  loopback = if self.pinentry == null then " --pinentry-mode loopback" else "";

  quiet = if self.quiet then " --quiet" else "";
    ...
    attrValues (
      mapAttrs (
        ...
        if value.decrypt != null then
          ...
          ''
        else if value.generate != null then
          let
  ...
  hostDefaultKey =
    if self.host.keys.default != null then [ "\"${self.host.keys.default}\"" ] else [ ];

  mandatoryKeys = hostDefaultKey ++ self.host.keys.mandatory;
  importMandatoryKeys = map (
    key: "${gnupg}/bin/gpg --homedir ${homedir}${quiet} --batch --import \"${key}\"\n"
  );

  makeUserDirectories = map (
    userName:
  ...
  startAgent =
    let
      gpgConfig = toFile "startAgent.config" ''
  ...
  generateDefaultKey =
    name: owner: path:
    let
      gpgConfig = toFile "generateDefaultKey.config" ''
        Key-Type: eddsa
        Key-Curve: Ed25519
        Key-Usage: sign,auth
        Subkey-Type: ecdh
  ...
  mayGenerateHostDefaultKey =
    if self.host.keys.default != null then
      ''
