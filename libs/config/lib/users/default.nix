...
let
  ...
  loadKey = { hostName, realName }:
    key:
    let
      convert = { contact ? null, date ? null, key, method, ... }:
        let
          contact' =
            if contact != null
            then
  ...
  UserConfiguration = Configuration "User"
    {
      ...
      wrap = { configuration, hasHomeManager, host, user, ... }:
        {
        ...
        // (
          ...
          then
            {
              home-manager.users.${user.name} = set.remove
                configuration
                [ "host" ];
            }
          else
  ...
  configure = { configuration, user, source, trusted, ... }:
    list.map
      UserConfiguration
      (
