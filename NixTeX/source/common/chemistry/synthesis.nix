...
let
  ...
  toParagraphs =
    ...
    type.matchPrimitiveOrPanic listOrString {
      ...
      list = list.map Paragraph listOrString;
    };
  ...
  mapProcedure =
    ...
    let
      ...
      result = type.matchPrimitiveOrPanic synthesis.procedure {
        lambda = list.map (paragraph: Paragraph' paragraph { endParagraph = "\\par"; }) (
          synthesis.procedure synthesis
        );
  ...
  formatAnalysis =
    ...
    let
      ...
      yield' = if isLambda yield then yield product else yield;
      inherit (yield') mass;
      ...
      relativeEquivalents =
        if equivalent != 1 then " (${physical.formatValue equivalent "equivalent"})" else "";
      # float
      relative =
        let
          relative = (yield'.relative or (amount / (reactant'.amount or amount) * 100)) * 1.0 * equivalent;
        in
        if relative > 100 then
      ...
      # string
      title = type.matchPrimitiveOrPanic reactant {
        set = if reactant.title != null then reactant.title else reactant.substance.ID;
        string = reactant;
      };

      # set
      reactant' = if set.isInstanceOf reactant then reactant else { };
    ...
    [
      (Paragraph'
        (
          if analysisOnly then
            if novel then [ novelText ] else [ ]
            ...
            [
              ...
              "  (${mass'}${
                  physical.formatValue {
                    value = amount'.value * purity;
                    precision = 2;
                  } "${amount'.prefix}mol"
                })"
              "  ${product.description or "Produkt"} mit \\mbox{${purity'}${
                  physical.formatValue {
                    value = relative * purity;
                    precision = 1;
                  } "percent"
                }-iger} Ausbeute"
              "    bezogen auf eingesetztes ${title}${relativeEquivalents}.${
                    if novel then " ${novelText}" else ""
                  }"
            ]
        )
        {
          endParagraph = if !hack then "\\par" else "\\par";
          /*
            endParagraph
             = if  !failure
    ...
    ++ (if failure then [ ] else reports);

  # Synthesis -> Product -> Document::Chunk.Paragraph | null
  formatProductChemicals =
    resources: syn: product:
    if product ? "chemicals" then
      let
        chemicals = type.matchPrimitiveOrPanic product.chemicals {
          null = [ ];
      ...
      if len > 1 then
        ...
        Paragraph' [ "Eingesetzt wurden ${formatChemical (list.head chemicals)}." ] {
          inherit endParagraph;
        }
      else
        null
    ...
    map (
      ...
      let
        ...
        body =
          ...
          ++ (
            if note != null then if lambda.isInstanceOf note then [ (note product') ] else [ note ] else [ ]
          )
          ++ (formatAnalysis resources synthesis product')
          ++ postAnalysis;

        lines' = if lines != null then "[${string lines}]" else "";

        body' = LaTeX (
          if substance != null && !noMolecule then
            [
    ...
    if solvent != null then
      let
        ...
        specialSolvents = {
          ...
          "Et2O" = {
            deu = "${acronyms.Et2O.as "etherische"}";
            eng = "${acronyms.Et2O.as "etheral"}";
          };
          "EtOH" = {
            deu = "${acronyms.EtOH.as "ethanolische"}";
            eng = "${acronyms.EtOH.as "ethanolic"}";
          };
          "MeOH" = {
            deu = "${acronyms.MeOH.as "methanolische"}";
            eng = "${acronyms.MeOH.as "methanolic"}";
          };
          "NH3" = {
            deu = "ammoniakalische";
            eng = null;
        ...
        acronym =
          ...
          .${solvent} or null;
      in
      if isString solvent then
        if special != null then
          "${special}${default casus "r"} ${molar}${title}\\-lösung"
        else if acronym != null then
          "${molar}${title}-Lösung in ${acronym.long}"
        else
          "${molar}${title}-Lösung in ${compound solvent}"
      else
        "${molar}${title}-Lösung in ${getTitle solvent}"
    else if dry && degased then
      "trockene${default casus "m"} und entgaste${default casus "m"} ${title}"
    else if dry then
      "trockene${default casus "m"} ${title}"
    else if degased then
      "entgaste${default casus "m"} ${title}"
    else
      "${molar}${title}";
  ...
  fixChemical =
    ...
    let
        ...
        if purity != 1.0 then
          ...
          type.matchPrimitiveOrPanic concentration {
            ...
            string =
              ...
              .${concentration} or (debug.panic "fixChemical" "Invalid Concentration »${concentration}«");
          };

      chemical' = chemical // {
        title = getFullTitle resources {
          inherit molar;
          title = getTitle {
            inherit
              acronym
      ...
      relative' =
        chemicals.${relative}
          or (debug.panic "fixChemical" "While calculating equivalent for ${name}: There is no chemical »${relative}«!");
    in
    if relative != null then
      chemical'
      // {
        equivalent = debug.warn "fixChemical" {
          text = "relative";
          data = {
  ...
  mapSyntheses =
    ...
    filterMap (
      ...
      let
        ...
        mapSubstances = string.concatMappedWith (
          substance:
          "${substance {
            mass = true;
            structure = true;
        ...
        product =
          if syn'.product or null != null then
            type.type.matchPrimitiveOrDefault syn'.product {
              lambda = syn'.product syn';
              list = list.map (product: type.callLambda product syn') syn'.product;
            } syn'.product
          else
            [ ];
        citeLiterature =
        ...
        formattedAnalysis = type.matchPrimitiveOrPanic product {
          null = [ ];
          list =
            if product != [ ] then
              ...
              [
                (Paragraph' [ "Es konnte kein Produkt erhalten werden." ] { endParagraph = ""; })
              ];
          set = formatAnalysis resources syn' (
            product // { chemicals = fixChemicals resources product.chemicals; }
          );
        };

        lines' = if syn'.lines or null != null then "[${string syn'.lines}]" else "";
      in
      if syn'.ignore or false then
        null
      else
        (
          if syn'.title or null != null then
            Heading'
              (
                if type.isSet syn'.title then
                  syn'.title
                else
                  {
                    bookmark = syn'.title;
              ...
              (
                if syn'.substances or null != null then
                  LaTeX (
                    [
                      "\\renewcommand{\\NumAtom}[2]{-[#1,,,draw=none]{\\scriptstyle#2}}%"
                  ...
                  (mapProcedure false syn') ++ formattedAnalysis
              )
              {
                clearPageOnLastQuarter = true;
              }
          else if syn'.substance or null != null then
            Heading'
              {
  ...
  formatDetails =
    chemical:
    if list.isInstanceOf chemical then
      concatWith "; " (list.map formatDetails' chemical)
    else
      formatDetails' chemical;
  ...
  formatDetails' =
    ...
    let
      details =
        (if chemical.substance != null then [ chemical.substance.ID ] else [ ])
        ++ (
          if chemical.volume != null then
            let
              precision =
                if chemical.kind != "solvent" then
                  null
                else if chemical.volume < 20 then
                  1
                else
                  0;

              volume = normaliseValue (chemical.volume / 1000.0) null;
              volume' = physical.formatValue {
                inherit (volume) value;
                inherit precision;
              } "${volume.prefix}litre";

              fullVolume = normaliseValue (chemical.times * chemical.volume / 1000.0) null;
              fullVolume' = physical.formatValue {
                inherit (fullVolume) value;
                inherit precision;
              } "${fullVolume.prefix}litre";
            in
            if chemical.times != null then
              [ "${string chemical.times}×${volume'}=${fullVolume'}" ]
            else
              [ volume' ]
          else
            [ ]
        ...
        ++ (
          if chemical.equivalent != null then
            debug.info "formatDetails'"
              {
                text = "Equivalent";
                data = chemical.equivalent;
              }
              (
                if chemical.kind == "catalyst" then
                  [
                    (physical.formatValue {
                      value = chemical.equivalent;
                      precision = null;
                    } "equivalent")
                  ]
                else
        ...
        ++ (if chemical.details != null then [ chemical.details ] else [ ]);
    in
  ...
  getUnitPrefix # :  integer -> string | !
    =
      ...
      if factor == -24 then
        "yokto"
      else if factor == -21 then
        "zepto"
      else if factor == -18 then
        "atto"
      else if factor == -15 then
        "femto"
      else if factor == -12 then
        "pico"
      else if factor == -9 then
        "nano"
      else if factor == -6 then
        "micro"
        ...
        "zetta"
      else if factor == 24 then
        "yotta"
      else
        error.throw "getUnitPrefix: Invalid Factor: ${string factor}";
  ...
  normaliseValue =
    ...
    let
      ...
      factor' =
        if factor != null then
          factor
        else if absInput >= 1000000000000000.0 then
          15
        else if absInput >= 1000000000000.0 then
          12
        else if absInput >= 1000000000.0 then
          9
        else if absInput >= 1000000.0 then
          6
        else if absInput >= 1000.0 then
          3
        else if absInput >= 1.0 then
          0
        else if absInput >= 0.001 then
          -3
        else if absInput >= 0.000001 then
          -6
        else if absInput >= 0.000000001 then
          -9
        else if absInput >= 0.000000000001 then
          -12
        else if absInput >= 0.000000000000001 then
          -15
        else
          0;
  ...
  getTitle =
    ...
    if title != null then
      ...
      substance.Name
    else if mixture != null then
      string.concatMappedWith getTitle "-" mixture
    else
      "???";
  ...
  formatName =
    ...
    else if chemical.the or null != null then
  ...
  formatChemical =
    ...
    in
    if details != "" then "${name} (${details})" else name;
  ...
  formatConcentration =
    ...
    in
    if concentration != null then
      "${
        (physical.formatValue {
          inherit (concentration') value;
          precision = 1;
        } "${concentration'.prefix}molar")
      }"
    else
        ...
        amount =
          debug.info "chemical"
            ...
            (
              if chemical'.amount != null then
                chemical'.amount
              else if chemical'.volume != null then
                if chemical'.mass != null then
                  debug.panic "Chemical" "Volume and mass are mutualy exclusive!"
                else if chemical'.concentration != null then
                  if isFloat chemical'.concentration then chemical'.volume * chemical'.concentration / 1000 else null
                else if chemical'.density != null && chemical'.dalton != null then
                  chemical'.purity * chemical'.volume * chemical'.density / chemical'.dalton
                else
                  null
              else if chemical'.mass != null then
                if chemical'.dalton != null then chemical'.mass / chemical'.dalton else null
              else
                null
            );
      };
    in
    if kind == "reagent" || kind == "catalyst" then
      if chemical''.amount == null then
        debug.panic "Chemical" {
          text = "Catalysts and Reagents need to have amount of substance (`amount`) given. Perhaps you want Material.";
          data = chemical'';
        }
      else if chemical''.equivalent == null && chemical''.relative == null then
