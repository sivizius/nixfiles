...
let
  Function = type.TypeConstructor "Function"
    (
      functionName:
      signature:
      {
        functionName = string.expect functionName;
        signature = debug.panic "Function"
          {
            ...
            nice = true;
            when = !(list.isInstanceOf signature)
          ...
          (
            argumentType:
            {
              inherit (argumentType) __traits__ __type__ __variant__
                ...
                expect instantiate instantiateAs;
            }
          )
          signature;
      }
