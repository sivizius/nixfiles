...
in
# :  D -> PreparedDocument<D> where D: Document
{
  configuration,
  resources,
  title,
...
let
  ...
  figures' = set.map (
    ...
    type.matchPrimitiveOrPanic figure {
      "path" = {
        src = figure;
        dst = "resources/figures/${name}-${path.getBaseName figure}";
      };
      "string" = {
        src = figure;
        dst = "resources/figures/${name}-${path.getBaseName figure}";
...
// {
  ...
  title = string.concatWords (
    list.filter (x: string.isInstanceOf x && !(string.isEmpty x)) (string.split "[[:space:]]+" title)
  );
}
