...
let
  TypeConstructor = type "TypeConstructor" {
    from =
      let
        __functor =
          { apply, instantiate, ... }@self:
          argument:
          let
            result = apply argument;
          in
          if lambda.isInstanceOf result then
            self
            // {
              apply = result;
            }
          else
            instantiate self result;
      in
      kind: apply: instantiate:
      TypeConstructor.instantiateAs kind {
        inherit apply instantiate;
        inherit __functor;
      };
  };
  ...
  applyFunction =
    ...
    if value'.isFunction or false then applyFunction value' else value';
  ...
  applyLambda =
    ...
    let
      value' = from value;
    in
    if lambda.isInstanceOf value' then applyLambda (self // { from = value'; }) else expect value';
  ...
  defaultFunctor =
    ...
    value:
    if lambda.isInstanceOf from then
      applyLambda self value
    else if from.isFunction or false then
      applyFunction from value
    else
      expect value;

  defaultInstanceOf = this: value: get value == this;
  ...
  enum =
    let
      constructEnum =
        ...
        variantName: value:
        let
          constructor = function: TypeConstructor "${enumName}::${variantName}" function (_: instantiate);
          enumName = __variant__;
          instantiate = self.instantiateAs variantName;
        in
        if type.isInstanceOf value then
          constructor value.expect
        else if lambda.isInstanceOf value then
          constructor value
        else
          instantiate value;
    in
    TypeConstructor "enum" (enumName: variants: { inherit enumName variants; }) (
      _:
      { enumName, variants, ... }:
      let
        from =
        ...
        match =
          ...
          let
            ...
            case' = if lambda.isInstanceOf case then case value' else case;
          ...
          debug.panic [ "enum" enumName "match" ]
            ...
            case';
        self = type enumName ((set.map (constructEnum self) variants) // { inherit from match; });
      in
      self
    );
  ...
  extend =
    ...
    let
      expect =
        value:
        if isInstanceOf value then
          value
        else if debug.Debug.isInstanceOf value then
          abort "${value}"
        else
      ...
      instantiate = instantiateAs null;
      instantiateAs =
        variantName: value:
        let
          value' = if set.isInstanceOf value then value else { inherit value; };
        in
        value'
        // {
    ...
    // {
      ...
      inherit
        ...
        mergeWith
        ;
      inherit expect instantiate instantiateAs;
    };
  ...
  format =
    ...
    if typeName != null then
      if variantName != null then "${typeName}::${variantName}" else typeName
    else
  ...
  get # :  T: Introspection @ T -> string
    =
      ...
      in
      if typeName != null then typeName else getPrimitive value;
  ...
  getType =
    ...
    in
    if this'.success then this'.value.__type__ or null else "never";

  getVariant = this: this.__variant__ or null;
  ...
  matchPrimitive' = function "matchPrimitive" [ any any any ] matchPrimitive;
  ...
  matchPrimitiveOrDefault' = function "matchPrimitiveOrDefault" [
    any
    any
    any
    any
  ...
  matchPrimitiveOrPanic' = function "matchPrimitiveOrPanic" [ any any any ] matchPrimitiveOrPanic;

  struct # :  string -> { string } -> type
    =
      let
        checkFields =
          structName: fields: signatures:
          let
          ...
          set.map
            (
              ...
              { expect, ... }@signature:
              expect (
                fields.${name} or signature.default or (debug.panic [ "struct" structName "checkFields" ] {
        ...
        (
          ...
          let
            self = type structName (
              values
              // {
                from = fields: self.instantiate (checkFields structName fields types);
              }
            );
          in
          self
    ...
    =
      ...
      type traitName (
        ...
        // {
          from =
            object:
            if type.isInstanceOf object then
              required:
              object
              // {
                __traits__ = object.__traits__ // {
                  ${traitName} = required // (methods required);
                };
              }
            else
              method:
