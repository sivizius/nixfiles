...
let
    ...
    let
      ...
      insert =
        ...
        if rest == [ ] then
          ...
          in
          if lessThan first second then
            [ first ] ++ rest
          else
            [ second ] ++ (insert lessThan first (list.tail rest));
    in
    lessThan: parts:
    if parts == [ ] then [ ] else insert lessThan (list.head parts) (insertionSort (list.tail parts));
  ...
  mergeSort =
    ...
    let
      ...
      half = len / 2;
      half' = len - half;
      left = list.generate (list.get parts) half;
    ...
    (list.fold
      (
        ...
        else if lessThan (list.head left) (list.head right) then
          {
            inherit right;
            left = list.tail left;
