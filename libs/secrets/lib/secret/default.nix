...
let
  ...
  decrypt = {
    toSecret = { encryptedFile, group, owner, ... }:
      {
  ...
  decryptGrafanaSecret = { encryptedFile, group ? null, owner ? "grafana" }:
    Secret.instantiateAs "decryptGrafanaSecret"
      {
        ...
        __functor = { encryptedFile, group, owner, ... } @ args:
          { hash, vaultBasePath, ... }:
          let
            secret = hash encryptedFile;
          in
    ...
    Secret.instantiateAs "decryptVariable"
      {
        ...
        __functor = { encryptedFile, environment, ... } @ args:
          { hash, vaultBasePath, ... }:
          let
            secret = hash encryptedFile;
  ...
  generateEnvFile = {
    toSecret = { group ? null, owner ? null, variables, ... }:
      {
  ...
  generateToken = {
    toSecret = { generator ? null, group ? null, length ? null, owner ? null, ... } @ args:
  ...
  Secret = type "Secret"
    {
      ...
      decryptGrafanaSecret' = encryptedFile:
