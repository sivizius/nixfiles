...
let
  ...
  renderHeading =
    ...
    if output == "LaTeX" then
      let
        label' = if label != null then [ "\\label{${label}}%" ] else [ ];
        header =
          [
            "\\${level'}[\\nolink{%"
            indentation.more
        ...
        clearPage' =
          if !concise && (clearDoublePage || level' == "chapter") then
            [ "\\cleardoublepage%" ]
          else if clearPage then
            [ "\\clearpage%" ]
          else if clearPageOnLastQuarter then
            [ "\\clearPageOnLastQuarter%" ]
          else
            [ ];
        rotate' = if rotate then [ "\\rotatePages%" ] else [ "\\unrotatePages%" ];
        before' =
          if before == null then
            [ ]
          else if list.isInstanceOf before then
            before
          else
            [ before ];
      in
