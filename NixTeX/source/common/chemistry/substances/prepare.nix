...
let
  ...
  format # :  Substance -> Options -> string
    =
      ...
      options:
      let
        options' = type.matchPrimitiveOrPanic options {
  ...
  prepare =
    ...
    type.matchPrimitiveOrPanic substances {
      ...
      path = prepare (source substances) (path.importScoped { inherit Substance; } substances);
        ...
        [
          {
            inherit (value) name;
            value = value // {
              __functor = { ... }: format value;
              __toString = { ... }: format value { };
              CodeID = format value {
                code = true;
                id = true;
              };
...
in
substances: list.toSet (prepare (context "substances") substances)
