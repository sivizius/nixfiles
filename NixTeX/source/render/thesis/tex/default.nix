...
let
  ...
  libs' = libs // {
    thesis =
      let
        cleardoublepage =
          configuration: if configuration.concise or false then "\\clearpage" else "\\cleardoublepage";
...
let
  ...
  originalityDeclaration =
    if thesis.originalityDeclaration or null != null then
      "resources/${path.getBaseName thesis.originalityDeclaration}"
    else
      null;
  backMatter =
    (toTex (content.backMatter or content.backmatter or null))
    ++ (if configuration.concise or false then [ ] else [ "\\cleardoublepage" ])
    ++ (
      if originalityDeclaration == null then
        [
  ...
  dependencies =
    ...
    ++ (
      if originalityDeclaration != null then
        [
          {
            src = thesis.originalityDeclaration;
            dst = originalityDeclaration;
          }
        ]
      else
        [ ]
