    ...
    let
      ...
      mapLocation =
        ...
        let
          ...
          directory = write.directory (foo (if location == "/" then domain else "${domain}${location}")) (
            set.map (
              fileName: page: if path.isInstanceOf page then "${page}" else path.toFile (foo fileName) "${page}"
            ) root
            // canary-txt
            // security-txt
          );
          ...
          canary-txt =
            ...
            set.ifOrEmpty (well-known."canary.txt" or { } != { }) {
              ".well-known/canary.txt" = path.toFile "canary.txt" (sign body signature);
            };
          ...
          security-txt =
            let
              ...
              format =
                ...
                ++ (
                  if list.isInstanceOf value then list.map "${name}: ${value}" value else [ "${name}: ${value}" ]
                );
            in
            set.ifOrEmpty (well-known."security.txt" or { } != { }) {
              ".well-known/security.txt" = path.toFile "security.txt" (
                sign (list.tail (list.concat (set.mapToList format body))) signature
              );
            };
        in
        if root != null then
          ...
          debug.panic "mapLocation" "Either root or redirect must be set!";

      sslProtocols = if allowLegacyTLS then "TLSv1.2 TLSv1.3" else "TLSv1.3";
    in
    {
      nginx = {
        ...
        statusPage = true;
        virtualHosts = set.mapNamesAndValues (
          subdomain: locations':
          let
          ...
          in
          {
            ...
            value = {
              ...
              locations = locations // (set.map (mapLocation fqdn) locations');
            };
          }
        ) websites;
      };
