{ lib, registries, ... }:
let
  inherit (lib.intrinsics) mapAttrs;
  ...
  keyOptions = {
    ...
    optional = mkOption {
      ...
      description = ''
        ...
        Usefull if multiple users should unlock the vault,
...
mkOption {
  type = types.submodule {
    options = {
      enable = mkEnableOption "confidential management of secrets.";
  ...
  apply =
    let
      mapSecrets = directory: mapAttrs (name: secret: secret // { path = "${directory}/${name}"; });
    in
    {
      vault,
      host,
    ...
    // {
      ...
      user = mapAttrs (user: secrets: mapSecrets "${vault}/user/${user}/" secrets.secrets) user;
    };
  description = ''
    The vault-module.
  '';
