...
let
  ...
  getFileName =
    fileName:
    let
      ext = string.slice ((string.length fileName') - extLength) extLength fileName';
      extLength = 4;
      fileName' = string.toString fileName;
    in
    if ext == ".nix" then fileName' else "${fileName'}/default.nix";
  ...
  extend =
    let
      ...
      extendWith =
        ...
        let
          self' = if attribute != null then addAttribute self attribute else self;
          self'' = if index != null then addIndex self' index else self';
        in
        if fileName != null then addFile self'' fileName else self'';
    in
    self: source:
    if Context.isInstanceOf source then
      source
    else
  ...
  from =
    ...
    Context.instantiate {
      ...
      fileName = if path.isInstanceOf root then root else "$";
