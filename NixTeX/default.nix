...
let
  ...
  collectDependencies/* :  Dependencies -> Dependencies | ! */ = list.fold
    (
      state:
      { dst, src ? null }:
  ...
  collectDirectories/* :  { string -> { ... } } -> { string -> null } */ = set.fold
    (
      directories:
      dst:
      src:
  ...
  collectFileSet = { local ? null, store ? null, ... }:
    if local != null
    then
      path.toStore local
    else if store != null
  ...
  collectFiles/* :  { string -> { path: string | path ? null ... } | string } -> [ string ] */ = set.fold
    (
      files:
      dst:
      src:
  ...
  collectTasks/* :  { string -> { cmd: string -> string ? null, ... } | string } -> [ string ] */ = set.fold
    (
      tasks:
      dst:
      src:
  ...
  toDerivation/* :  System -> Document -> derivation */ = system:
    { name, dependencies, ... }:
    let
  ...
  mapToDerivations/* :  System -> [ Document ] -> { string -> derivation } */ = system:
    let
  ...
  mapToPackages/* :  [ Document ] -> { ... } -> { string -> derivation } */ = documents:
    environment:
