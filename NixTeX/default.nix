...
let
  ...
  collectDependencies # :  Dependencies -> Dependencies | !
    = list.fold (
      ...
      if src == null || dst == null then
        state
      else if state.${dst} or null == null then
        state // { ${dst} = src; }
      else if state.${dst} == src then
        state
      else if lambda.isInstanceOf state.${dst} && lambda.isInstanceOf src then
        state # Cannot compae tasks :c
      else
        debug.panic "collectDependencies" "Cannot set ${dst}, because it is already set!"
    ) { };
      ...
      type.matchPrimitiveOrPanic src {
        ...
        set =
          ...
          if src' != null then
            ...
            files;
        string = debug.warn "collectFiles" {
  ...
  collectTasks # :  { string -> { cmd: string -> string ? null, ... } | string } -> [ string ]
    = set.fold (
      tasks: dst: src:
      if set.isInstanceOf src && src.cmd or null != null then
        tasks ++ [ (src.cmd "$out/${dst}") ]
      else if lambda.isInstanceOf src then
        tasks ++ [ (src "$out/${dst}") ]
      else
        tasks
    ) [ ];
  ...
  toDerivation # :  System -> Document -> derivation
    =
      ...
      let
        ...
        directories = list.map (dictionary: "mkdir -p \"$out/${dictionary}\"") (
          set.names (collectDirectories dependencies')
        );
        links = list.map ({ dst, src }: "ln -s \"${src}\" \"$out/${dst}\"") (collectFiles dependencies');
    ...
    =
      ...
      target.System.mapStdenv (
        ...
        mapToDerivations system (
          list.map (
            documentPath: library.import documentPath ({ inherit core nixtex; } // environment) "tex"
          ) documents
        )
      );
in
