...
let
  ...
  collect =
    ...
    ++ (list.map PeerConfiguration peers);

  extendName =
    networkName: peerName: if networkName != null then "${peerName}.${networkName}" else peerName;

  prepare =
    environment: host: network:
    if set.isInstanceOf network then
  ...
  prepareNetwork =
    ...
    let
      ...
      loadPeer = peer: peers.load peer environment;
      mapPeers =
        networkName: peers:
        list.concat (
          set.mapToList (
            ...
            let
              ...
              name = extendName networkName peerName;
              namedPeer = {
                inherit name;
              } // loadedPeer;
            ...
            in
            if Peer.isInstanceOf loadedPeer then
    ...
    }:
    {
      ...
      peers = type.matchPrimitiveOrPanic peers {
        list = mapPeers' (
          list.imapValuesToSet (index: peer: {
            name = string index;
            value = peer;
          }) peers
        );
