...
in
{
  compare/* :  string -> string -> int */ = intrinsics.compareVersions;

  deriveVersion/* :  ? */ = dateTime:
    let
  ...
  language/* :  string? */ = intrinsics.langVersion or null;

  main/* :  string -> string */ = version: "${string version.major}.${string version.minor}";

  nix/* :  string? */ = intrinsics.nixVersion or null;

  parseDerivationName/* :  string -> { name: string, version: string } */ = intrinsics.parseDrvName
    or  (
    derivationName:
    let
  ...
  split'/* :  string -> [ string ] */ = intrinsics.splitVersion
    or  (string.splitAt "[.]");
}
