    ...
    chemistry = libs.chemistry // {
      compound =
        ...
        let
          ...
          parts' = list.filter string.isInstanceOf parts;
        in
        string.concat parts';
      formatNMRnucleus = nucleus: nucleus;
    };
    physical = libs.physical // {
      formatValue =
        value: unit:
        let
          value' =
            if set.isInstanceOf value then
              if value ? value then string value.value else "<value>"
            else
              string value;
          unit' = if string.isInstanceOf unit then unit else "<unit>";
        in
        "${value'} ${unit'}";
    };
  };
      ...
      line' =
        (list.fold
          (
            ...
            else if stack == [ ] || (list.head stack) != (list.head token) then
              {
                stack = token ++ stack;
                text = "${text}"; # \textit{
              }
