...
let
  ...
  Configuration =
    let
      ...
      defaultWrap = { configuration, ... }: configuration;
      ...
      toLegacy =
        ...
        let
          fix =
            source: value:
            let
              result = fixDictionary source (set.expect value);
            in
            debug.debug "fix"
              {
            ...
            // (set.ifOrEmpty (result.secrets != { }) {
              vault = (result.value.vault or { }) // {
                secrets =
                  if result.value ? vault.secrets then
                    Vault.update result.value.vault.secrets result.secrets
                  else
                    result.secrets;
              };
            });
          ...
          fixDictionary =
            ...
            debug.debug "fixDictionary"
              ...
              (
                ...
                in
                if debug.Debug.isInstanceOf result then
                  "${result}"
                else
                  {
                    secrets = Vault.update secrets result.secrets;
          ...
          fixSet =
            ...
            debug.debug "fixSet"
              ...
              (
                if derivation.isInstanceOf' value || value._type or null != null || HTML.isInstanceOf value then
                  {
                    inherit value;
                    secrets = { };
                  }
                else if type.getType value == null then
                  fixDictionary source value
                else if debug.Debug.isInstanceOf value then
                  abort "${value}"
                else if Secret.isInstanceOf value then
        ...
        let
          ...
          imports = type.matchPrimitiveOrPanic configuration {
    ...
    type "Configuration" {
      from =
        ...
        Configuration.instantiateAs variant (
          config
          // {
  ...
  LegacyConfiguration = Configuration "Legacy" {
  ...
  collect =
    ...
    type.matchPrimitiveOrPanic configuration {
      lambda = [ this ];
      list = list.concatMap (configuration: collect (this // { inherit configuration; })) configuration;
      null = [ ]; # Just Ignore
  ...
  mapToLegacy =
    ...
    let
      ...
      configurations' = debug.info "configurations'" { text = "configurations"; nice = true; data = configurations; } list.map (
        { toLegacy, ... }@cfg:
        toLegacy theVault {
      ...
      imports = debug.info "imports" { text = "configurations'"; nice = true; data = configurations'; } list.concatMap ({ imports, ... }: imports) configurations';

      merge =
        lhs: rhs:
        let
          merge = type.matchPrimitiveOrDefault lhs {
            list = mergeLists;
            set = mergeAttrs;
          } mergeEqual;
        ...
        in
        debug.panic "merge" {
          text = "Cannot merge values of different type";
          when = type.getPrimitive lhs != type.getPrimitive rhs;
          data = {
            inherit lhs rhs;
          };
        } value;
      ...
      mergeAttrs = set.fold' (
        ...
        lhs
        // {
          ${name} = if set.hasAttribute name lhs then merge lhs.${name} value else value;
        }
      );

      mergeLists = lhs: rhs: lhs ++ rhs;

      mergeEqual =
        lhs: rhs:
        debug.panic "mergeEqual" {
          text = "Cannot values, because they are not equal!";
          data = {
            inherit lhs rhs;
          };
          when = lhs != rhs;
      ...
      mergeModules = lhs: rhs: let
        callModule = module: type.matchPrimitiveOrPanic module {
          set = module;
          lambda = module { };
        };
      ...
      imports' = debug.info "imports" {
        text = "merge imports";
        data = imports;
        nice = true;
      } (list.fold mergeModules { } imports);
