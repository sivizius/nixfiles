...
let
  ...
  after =
    ...
    in
    if lYear == rYear then
      if lMonth == rMonth then
        if lDay == rDay then
          if lHour == rHour then
            if lMinute == rMinute then lSecond > rSecond else lMinute > rMinute
          else
            lHour > rHour
        else
          lDay > rDay
  ...
  format # :  D: ToDateTime @ D -> string -> string -> string
    =
      dateTime: language: format:
      let
        date = from dateTime;
        handle =
          token:
          if list.isInstanceOf token then
            let
              parts = string.match "%([-_0+^#])*([EO]?[A-Za-y%]|(:{0,3})z)" (list.head token);
              config =
                list.fold
                  (
                    state: token:
              ...
              pad =
                ...
                if padding != "" && len > 0 then
                  "${padding'}${text}"
                else if config.plus && len < 0 then
                  "+${text}"
                else
                  text;
              adjustCase =
                text: maybeLower:
                if maybeLower && config.opposite then
                  string.toLowerCase text
                else if config.upper then
              ...
              isoYear = if startOfWeek <= 0 then date.year - 1 else date.year;
              ...
              zone3 =
                if zoneSeconds != 0 then
                  zone2
                else if zoneMinutes != 0 then
                  zone1
                else
                  zoneHours';
            in
      ...
      in
      if date != null then
        list.fold (result: token: "${result}${handle token}") "" (
  ...
  formatDateTime # :  D -> string -> string
    ...
    =
      ...
      let
        ...
        pad # :  int -> string
          =
            value: if value < 10 then "0${string value}" else string value;
      in
      if date != null then
    ...
    =
      ...
      (type.matchPrimitiveOrPanic dateTime {
        ...
        string =
          ...
          in
          if dateTime' != null then parseISO8601 dateTime else parseDateTime dateTime;
      })
      // {
        __toString =
          let
  ...
  getDayName # :  int -> string -> string
    =
      ...
      let
        ...
        days' = if language == null then days.eng else (days.${language} or days.eng);
      in
  ...
  getMonthName # :  int -> string -> string
    =
      ...
      let
        ...
        months' = if language == null then months.eng else (months.${language} or months.eng);
      in
  ...
  parseUnixTime # :  int -> int | float | null | string -> DateTime
    =
      unix: zoneOrName:
      let
        zone = type.matchPrimitiveOrPanic zoneOrName {
        ...
        zoneName = if string.isInstanceOf zoneOrName then zoneOrName else null;
        ...
        positiveEra =
          if daysSinceMarchZero >= 0 then daysSinceMarchZero else daysSinceMarchZero - daysPerEra + 1;
        era = positiveEra / daysPerEra;
        dayOfEra = daysSinceMarchZero - era * daysPerEra;
        yearOfEra =
          (dayOfEra - dayOfEra / (daysPerCycle - 1) + dayOfEra / daysPerCentury - dayOfEra / (daysPerEra - 1))
          / daysPerYear;
    ...
    =
      ...
      let
        ...
        toInteger' # :  string | null -> int | null
          =
            value: if value != null then integer value else null;
      in
      if string.isInstanceOf iso8601 && matched != null then
        DateTime {
