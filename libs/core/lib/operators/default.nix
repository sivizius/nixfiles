...
}:
{
  add =
    x: y:
    if number.isInstanceOf x && number.isInstanceOf Y then
      intrinsic.add x y
    else
      debug.panic "add" {
  ...
  and =
    x: y:
    if bool.isInstanceOf x && bool.isInstanceOf Y then
      x && y
    else
      debug.panic "and" {
