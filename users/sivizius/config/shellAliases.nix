...
in
{
  ...
  '';
  l = "${registries.nix.eza}/bin/eza -l@ah";
  inherit lt;
  man = ''
    :(){
      unset -f :
  ...
  '';
  use = "useFrom https://github.com/NixOS/nixpkgs/archive/master.tar.gz";
  useFrom = ''
    :(){
      unset -f :
      nixpkgs="$1"
      shift 1
      mkdir -p "$HOME/.cache/use/"
      for p in $@
      do
        echo "$p@$nixpkgs" >> "$HOME/.cache/use/history.log"
      done
