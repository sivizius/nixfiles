...
let
  ...
  mapToLegacy = { configurations, environment, host, modules, ... }:
    let
      theVault = Vault { };
      ...
      modules' = (set.values modules)
        ++ [ web.module vault ]
        ++ configurations';

      config = debug.info "mapToLegacy"
        {
          text = "modules'";
          data = modules';
          nice = true;
  ...
  sortUniqueChecked = configurations:
    let
