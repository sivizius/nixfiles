...
let
  ...
  formatJournal' =
    substance:
    if substance ? simple then "\\ch{${substance.simple}}" else compound.format substance.title;

  formatJournal =
    product:
    if product.ignore or false then
      null
    else if product.substance != null then
      if list.isInstanceOf product.substance then
        string.concatWith " + " (list.map formatJournal' product.substance)
      else
        formatJournal' product.substance
    else if product ? title then
      "\\textbf{${product.title}}"
    else
      "a???";
  ...
  idToFloat =
    ...
    let
      ...
      letters' = list.generate (x: {
        name = list.get letters x;
        value = 0.01 * x;
      }) (list.length letters);
      lookUpLetter = list.toSet letters';
    in
    if matched != null then integer (list.get matched 0) + lookUpLetter.${list.get matched 1} else 0;
  ...
  formatJournalEntry =
    entry:
    if entry.product != null then
      let
        getSimple =
          input:
          if input.substance or null != null then
            input.substance.simple or "b???"
          else if input.acronym or null != null then
            "<${input.acronym}>"
          else if input.simple or null != null then
            input.simple
          else
            null;
        mapInput = list.map (
          input:
          if list.isInstanceOf input then
            let
            ...
            in
            if simple != null then "\\ch{${simple}}" else "c???"
        );
        inputs =
          if entry.reaction != null then
            let
            ...
            "…";
      in
      debug.debug "formatJournalEntry" "${string entry.id}: ${entry.product}"
        "${string entry.id} & ${inputs} & ${entry.product}\\\\"
    else
      debug.debug "formatJournalEntry" "${string entry.id}: ${entry.title}"
        "${string entry.id} & \\multicolumn{2}{l}{${entry.title}} \\\\";

  generateJournal =
    syntheses: arguments:
    let
      syntheses' =
        if path.isInstanceOf syntheses then import syntheses ({ core = null; } // arguments) else syntheses;
    in
    document.LaTeX (
      [
