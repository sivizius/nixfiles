...
let
  ...
  from = function "from" [ any Derivation ] (
    {
      name,
      builder,
      system,
  ...
  fromStrict = function "fromStrict" [ any any ] (
    ...
    Derivation.instantiate (intrinsics.derivationStrict drvAttrs)
  );
  ...
  isInstanceOf =
    ...
    || (debug.error "isInstanceOf" {
      ...
      data = {
        keys = set.names value;
        inherit (value) name pname outPath;
      };
      when = legacy;
    } legacy);

  isInstanceOf' = value: (type.defaultInstanceOf "Derivation" value) || (isLegacy value);
