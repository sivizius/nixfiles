...
set.map
  (
    key: value:
    if list.isInstanceOf value then
      debug.warn [ fileName key ] "Use <libcore>.${string.concatWith "." value} instead!" (
        list.fold (libs: name: libs.${name}) libs value
      )
    else
      value
  )
  {
    ...
    __toPath = debug.warn [ fileName "__toPath" ] "Deprecated, use `/. + \"/absolute/path\"`!" (
      x: /. + x
    );
    __toXML = [
      "expression"
    ...
    __trace = debug.warn [
      fileName
      "__trace"
    ] "Use printers of <libcore>.debug instead!" intrinsics.trace;
    __tryEval = [
      "expression"
      "tryEval"
    ];
