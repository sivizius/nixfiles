...
let
  ...
  replaceNewline = text: string.replace [ "\n" ] [ "\\noexpand\\linebreak " ] (string.trim text);
  ...
  renderDedication =
    ...
    let
        ...
        let
          transliteration =
            let
              lines =
                let
                  lines = list.filter (line: line != "") (
                    list.map string.trim (string.splitLines epigraph'.text.latin)
                  );
                ...
                in
                if length == 1 then [ "[${first}]%" ] else [ "[${first}\\linebreak%" ] ++ middle ++ [ "${last}]%" ];
            in
            if epigraph'.text.language != null && epigraph'.text ? latin then
              [
                "\\par%"
                "{\\tiny%"
                indentation.more
          ...
          text =
            let
              lines =
                let
                  lines = list.filter (line: line != "") (
                    list.map string.trim (string.splitLines epigraph'.text.original)
                  );
                  first = list.head lines;
                  last = list.foot lines;
                  middle = list.map (line: "${line}\\linebreak%") (list.body (list.tail lines));
                ...
                if list.length lines == 1 then
                  if epigraph'.text.language != null then
                  ...
                  ++ (
                    if epigraph'.text.language != null then [ "\\selectlanguage{${epigraph'.text.language}}%" ] else [ ]
                  )
                  ++ [ "${first}\\linebreak%" ]
                  ++ middle
                  ++ [
          ...
          author =
            if epigraph'.author != null then
              if epigraph'.author.about != null then
                [
      ...
      epigraphTeX' =
        if epigraph != null then
          [
            "{~\\vfill\\hfill\\parbox[][][t]{0.666\\linewidth}{{%"
            indentation.more
            "\\tolerance 500%"
