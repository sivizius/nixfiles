...
let
  ...
  formatTeX =
    ...
    urls.formatTeXboxed "tel:${number}" (
      if number'.country != null then
        if number'.area != null then
  ...
  parse =
    ...
    let
      ...
      country =
        if international != null then
          "+${list.head international}"
        else if national != null then
          "0"
        else
          null;
      ...
      areaAndSuffix =
        if country != null && country != "0" then
          {
            "+49" =
              let
    ...
    type.matchPrimitiveOrPanic number {
      ...
      string = {
        inherit country;
        inherit (areaAndSuffix) aread suffix;
      };
    };
in
{
