...
let
  ...
  evaluateList = { ... }: { bibliography, ... }@state: { items, ... }: state;
  ...
  renderList =
    ...
    ++ (list.map (
      ...
      type.matchPrimitiveOrPanic item {
        ...
        set =
          let
            label = if item.label or null != null then "[${item.label}]" else "";
            slides = if item.slides or null != null then "<${item.slides}>" else "";
          in
