...
let
  ...
  formatCommon = { authors, doi, title, url, ... }:
    [
  ...
  getFormatter = shortJournalNames:
    variant:
    let
      JournalArticle = { issue, pages, ... }:
        let
          journalName =
    ...
    in
      {
        Book = { date, isbn, publisher, series, ... }:
          (formatDate date)
          ++ [
            (toLine "isbn" isbn)
        ...
        ConferenceArticle = { conference, pages, ... }:
