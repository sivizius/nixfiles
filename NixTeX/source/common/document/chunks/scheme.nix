...
let
  ...
  mapChemeToken =
    token:
    let
      phantom = text: if token.phantom or false then "\\phantom{${text}}" else text;
    in
    if token ? plus then
      [ "\\+" ]
    else if token ? arrow then
      let
        optionals = list.map (
          argument:
          type.matchPrimitiveOrPanic argument {
            ...
            string = "[\\tiny{${argument}}]";
            set =
              if argument ? text then
                let
                  pos = if argument ? pos then ".${argument.pos}" else "";
                in
        ...
        fromTo = if from != "" || till != "" then "(${string from}--${string till})" else "";
        angle = token.config.angle or "";
        length = token.config.length or "";
        config = if angle != "" || length != "" then "[${string angle},${string length}]" else "";
      in
      ...
      if token ? substance then
        ...
        let
          movPart = type.matchPrimitiveOrPanic token.movPart {
            list = string.concat token.movPart;
            string = token.movPart;
          };
          movPart' = if token ? movPart then [ "\\chemmove{${movPart}}" ] else [ ];
          figPart = type.matchPrimitiveOrPanic token.figPart {
  ...
  mapChemeLine =
    ...
    list.concatMap (
      ...
      ++ (
        let
          skip' = if skip != null then "[${string skip}\\normalbaselineskip]" else "";

        in
        [ "\\chemnameinit{}\\\\${skip'}%" ]
      )
      ++ (if only != null || uncover != null then [ "}%" ] else [ ])
    ) line;
  ...
  renderCheme =
    ...
    if output == "LaTeX" then
      ...
      ++ (putCaption scheme)
      ++ (if label != null then [ "\\labelScheme{${label}}%" ] else [ ])
      ++ [
        indentation.less
        "\\end{scheme}"
      ]
...
in
{
  ...
  Cheme = {
    Arrow = arrow: config: arguments: { inherit arrow config arguments; };
    Plus = {
      plus = null;
    };
    __functor =
