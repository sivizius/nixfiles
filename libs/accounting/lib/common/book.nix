...
let
  ...
  checkTransactionAccounts # :  { Account } -> { Account } -> Transaction -> [ string ]
    =
      allAccounts: transactionAccounts: transaction:
      let
        unknownAccounts = getUnknownAccounts allAccounts transactionAccounts;
      in
      if unknownAccounts == [ ] then
        transactionAccounts
      else
  ...
  collectAccounts = list.fold (
    accounts:
    { uid, ... }@account:
    if accounts.${uid} or null == null then
      accounts
      // {
        ${uid} = account;
      }
    else
      debug.panic "Book" "Duplicate of account UID: ${uid}"
  ) { };

  creditAccounts # :  { Account } -> { Amount } -> { reference: R } -> { Account } | !
    # where R: Any
    =
      allAccounts: creditAccounts: transaction:
      set.fold (
        allAccounts: accountUID: amount:
        let
      ...
      ) allAccounts (checkTransactionAccounts allAccounts creditAccounts transaction);

  debitAccounts # :  { Account } -> { Amount } -> { reference: R } -> { Account } | !
    # where R: Any
    =
      allAccounts: debitAccounts: transaction:
      set.fold (
        allAccounts: accountUID: amount:
        let
          this = allAccounts.${accountUID};
      ...
      ) allAccounts (checkTransactionAccounts allAccounts debitAccounts transaction);

  flat =
    source:
    let
      source' = if source != null then "${source}-" else "";
    in
    list.concatMap (
      {
        __type__,
      ...
      .${__type__} or (debug.panic [ "Book" "flat" ] {
  ...
  getSectionUIDs =
    ...
    list.concatMap (
      ...
      {
        Account = [ ];
        Section = [ "${source'}${uid}" ] ++ getSectionUIDs "${source'}${uid}" body;
      }
      .${__type__} or (debug.panic [ "Book" "getSectionUIDs" ] {
        text = "Either Account or Section expected, got:";
        data = entry;
      })
    );
  ...
  getUnknownAccounts # :  { Accounts } -> [ string ] -> [ string ]
    =
      allAccounts: checkAccounts:
      list.filter (accountUID: allAccounts.${accountUID} or null == null) (set.names checkAccounts);

  lookUp =
    let
      find =
        dictionary: key: prefixes:
        if key != null then
          list.fold (
            result: prefix:
            let
          ...
          null;
      haz = dictionary: key: if key != null then dictionary.${key} or null else null;
          ...
          (
            ...
            let
              regexMatch =
                ...
                || (
                  description != null
                  && regex.description or null != null
                  && ((string.match regex.description description) != null)
                );

              checkParam =
                field: value:
                (client.${field} or null != null)
                && (if list.isInstanceOf value then list.find client.${field} value else client.${field} == value);

              paramMatch = matches != { } && (set.any checkParam matches);
            in
            debug.info [ "Book" "lookUp" "match" ] {
              text = "Check ${account}";
  ...
  makeLookUp =
    ...
    let
      insert =
        { uid, ... }@account:
        dictionary: key:
        if dictionary.${key} or null == null then
  ...
  sortJournal =
    journal:
    list.imap (uid: transaction: transaction // { inherit uid; }) (
      list.sort ({ dateTime, ... }: other: time.before dateTime other.dateTime) journal
    );

  HTML =
    let
      flat = body: if list.isInstanceOf body then list.concatMap flat body else [ body ];

      mapAttributes =
        attributes:
        string.concatMapped (key: " ${key}=\"${string attributes.${key}}\"") (set.names attributes);

      Block =
        name: attributes: body:
        [
          "<${name}${mapAttributes attributes}>"
          indentation.more
        ]
        ++ (flat body)
        ++ [
  ...
  harmonise =
    ...
    ) self.inner (set.names inner);

  toAmounts = set.mapToList (currency: value: Amount value currency);
    ...
    let
      renderSections =
        ...
        list.concatMap (
          ...
          .${__type__} or (debug.panic [ "Book" "toHTML" ] {
            text = "Either Account or Section expected, got:";
            data = entry;
          })
        );
            ...
            ++ [
              ...
              (HTML.Block' "ul" (
                list.map (
                  ...
                  HTML.Line' "li" (
                    let
                      render = clients: string.concatCSV (set.mapToList (uid: amount: "(${uid}: ${amount})") clients);
                    in
