...
let
  ...
  listSectionTotal' # :  int -> T -> string
    ...
    =
      ...
      in
      if entries != "" then "  ${string.repeat "  " depth}${title}:\n${entries}" else null;
    ...
    =
      ...
      concatLines' (
        ...
        let
          ...
          total = if negative then -entry.total else entry.total;
        in
        if entry ? name then
          "  ${padNameValue "${string.repeat "  " depth}${entry.name}" total currency 2}"
        else if filterSection entry then
  ...
  formatBalance # :  Self -> Date -> string
    =
      dateTime: self: formatBalance' (balance dateTime self);
      ...
      let
        ...
        listSections # :  [ Transaction ] -> bool -> int -> T -> string
          ...
          =
                  ...
                  let
                    listTransactions # :  [ Transaction ] -> [ { amount: Amount, reference: int, ... } ] -> string
                      = list.concatMap (
                        { amount, reference, ... }:
                        let
                          transaction = list.get allTransactions reference;
                        in
                        "\n${padding}        ${padNameValue transaction.description amount currency (8 + 2 * depth)}"
                      );
                  in
                  if debit.journal != [ ] || credit.journal != [ ] then
                    ''
                      ${padding}    ${name}
            ...
            in
            concatLines' (
              entry: if entry ? name then formatAccount entry else listSections' allTransactions (depth + 1) entry
            );

        expensesAndRevenues = list.partition ({ total, ... }: total >= 0) outcome.accounts;
        expenses = {
          title = accountNames.expenses;
          accounts = expensesAndRevenues.wrong;
        };
        revenues = {
          title = accountNames.revenues;
          accounts = expensesAndRevenues.right;
        };
      in
      ''
  ...
  formatOutcome # :  Self -> { from: D, till: D } -> string
    # where D -> DateTime
    =
      { from, till }@period: self: formatOutcome' (outcome period self);
