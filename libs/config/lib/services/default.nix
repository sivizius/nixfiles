...
let
  ...
  ServiceConfiguration = Configuration "Service" {
    call =
      { environment, host, ... }:
      {
        arguments,
  ...
  load = source: environment: configurations.load source environment constructors Service;

  prepare =
    environment: host: services:
    if set.isInstanceOf services then
      set.map (prepareService environment host) services
    else
      debug.panic "prepare" "The option `services` must be a set.";

  prepareService =
    environment: host: name: service:
    {
