...
[
  ...
  (Section "Aktiva" "Aktiva" [
    (
      if level >= Level.Klein then
        Section "A" "Anlagevermögen" [
          (
            if level >= Level.Mittel then
              Section "I" "Immaterielle Vermögensgegenstände" [
                (Account "1" {
                  title = "Selbst geschaffene gewerbliche Schutzrechte und ähnliche Rechte und Werte";
                })
                (Account "2" {
                  title = "Entgeltlich erworbene Konzessionen, gewerbliche Schutzrechte und ähnliche Rechte und Werte sowie Lizenzen an solchen Rechten und Werten";
                })
          ...
          (
            if level >= Level.Mittel then
              Section "II" "Sachanlagen" [
                (Account "1" {
                  title = "Grundstücke, grundstücksgleiche Rechte und Bauten einschließlich der Bauten auf fremden Grundstücken";
                })
          ...
          (
            if level >= Level.Mittel then
              Section "III" "Finanzanlagen" [
                ...
                (Account "4" {
                  title = "Ausleihungen an Unternehmen, mit denen ein Beteiligungsverhältnis besteht";
                })
          ...
          (
            if level >= Level.Mittel then
              Section "II" "Forderungen" [
                ...
                (Account "3" {
                  title = "Forderungen gegen Unternehmen, mit denen ein Beteiligungsverhältnis besteht";
                })
                (
                  if level >= Level.Gross then
          ...
          (
            if level >= Level.Mittel then
              Section "III" "Gewinnrücklagen" [
                ...
                (Account "2" {
                  title = "Rücklage für Anteile an einem herrschenden oder mehrheitlich beteiligten Unternehmen";
                })
    ...
    (
      if level >= Level.Mittel then
        Section "C" "Verbindlichkeiten" [
          ...
          (Account "5" {
            title = "Verbindlichkeiten aus der Annahme gezogener Wechsel und der Ausstellung eigener Wechsel";
          })
          (Account "6" { title = "Verbindlichkeiten gegenüber verbundenen Unternehmen"; })
          (Account "7" {
            title = "Verbindlichkeiten gegenüber Unternehmen, mit denen ein Beteiligungsverhältnis besteht";
          })
          (
            if level >= Level.Gross then
              ...
              Account "8" {
                title = "Sonstige Verbindlichkeiten, davon aus Steuern, davon im Rahmen der sozialen Sicherheit";
              }
          )
        ]
      else
  ...
  (Section "GuV" "Gewinn- und Verlustrechnung" (
    ...
    else if method == "Umsatz" then
      [

      ]
    else if method == "Gesamt" then
      [
      ]
    else
      debug.panic "GuV" ''
