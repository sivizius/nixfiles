...
{ configuration, resources, ... }:
{
  ...
  src = path.fromSet "acronyms.lua" (
    name: value:
    let
      escape = text: string.trim (string.replace [ "\\" "\n" ] [ "\\\\" " " ] text);
      textDeu =
        if set.isInstanceOf value.text.deu then
      ...
      textEng =
        if value.text ? eng && set.isInstanceOf value.text.eng then
      ...
      sortedBy =
        if value ? "sortedBy" then
          if set.isInstanceOf value.sortedBy then
            "\n      sortedBy\n = {\n            deu = \"${
              escape value.sortedBy.deu or ""
          ...
          "";
      bookmarkAs =
        if value ? "bookmarkAs" then "\n      bookmarkAs = \"${escape value.bookmarkAs}\"," else "";
      chemical = if value.data ? "struct" then "\n            \"${escape value.data.struct}\"," else "";
      short =
        if set.isInstanceOf value.data.short then
          "{\n              deu = \"${escape value.data.short.deu or ""}\",\n              eng = \"${
            escape value.data.sorshortt.eng or ""
          },\"\n            }"
        else
