...
let
  ...
  formatGit = { name, domain, isGitea ? false, ... }:
    let
      icon = {
        "github" = icons.github;
  ...
  formatWikipedia = { language ? "en", name }:
  ...
  mapPhone = { cell ? null, fax ? null, home ? null }:
    [ ]
...
let
  items = (list.ifOrEmpty (matrix != null) (formatMatrix matrix))
