...
let
  initialState = {
    ...
    notes =
      ...
      in
      {
        ...
        pages = list.ifOrEmpty' (titleNotes != null) [
          {
            title = "Title Page";
            level = 0;
          }
  ...
  evaluatedState = list.fold (
    state: part:
    if set.hasAttribute part content then
      evaluate { inherit configuration resources; } (state // { context = [ ]; }) content.${part}
    else
      state
  ) initialState evaluationOrder;
    ...
    in
    {
      ...
      src = path.toFile "${name}.md" (
        string.concatMappedLines (
          ...
          if title != null then
            let
              title' = if list.isInstanceOf title then string.concatWords title else string title;
            in
            if level == 0 then "# ${title'}" else "## ${title'}"
          else
            note
        ) notes.pages
      );
...
// {
  dependencies =
    ...
    ++ (list.ifOrEmpty' (evaluatedState.notes.label != 0) [
      pdfpc
      notes
    ])
    ++ [
  ...
  resources = resources // {
    substances = substances.finalise resources.substances evaluatedState.substances;
  };
