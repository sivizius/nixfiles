...
let
  ...
  addReferences # :  [ { ... } ] -> [ { reference: int, ... } ]
    = list.imap (reference: this: this // { inherit reference; });
  ...
  balance # :  Self -> ~DateTime-> Balance | !
    =
      ...
      let
        ...
        journal = addReferences (
          list.filter ({ dateTime, ... }: !(time.after dateTime balanceTime')) self'.journal
        );
      in
      balance' (
        self'
  ...
  balanceToTransaction # :  Balance -> Transaction
    =
      ...
      // {
        inherit dateTime;
        debit = set.filterValue ({ total, ... }: total > 0) accounts;
        credit = set.mapValues (total: -total) (set.filterValue ({ total, ... }: total < 0) accounts);
      };

  format = library.import ./format.nix libs {
    inherit
      balance
      balance'
      ...
      let
        flatSections # :  { title: string, accounts: T, ... } -> [ { id: string, name: string, ... } ]
          ...
          =
            this:
            type.matchPrimitiveOrPanic this {
              set = if this ? accounts then flatSections' this.accounts else [ this ];
            };

        flatSections' # :  T -> [ { id: string, name: string, ... } ]
          # where T: [ { id: string, name: string, ... } | { title: string, accounts: T, ... } ]
          =
            this: list.concatMap flatSections (list.expect this);
      ...
      let
        splitTransaction # :  [ Transaction ] -> ~DateTime -> ~DateTime -> { current: [ Transaction ], before: [ Transaction ] }
          =
            journal:
            let
              parts = list.partition ({ dateTime, ... }: time.before (time.from dateTime) from') journal;
              parts' = list.filter ({ dateTime, ... }: !(time.after (time.from dateTime) till')) parts.wrong;
            in
            {
              before = parts.right;
              current = parts';
        ...
        initialTransaction = balanceToTransaction (
          balance' (
            self'
            // {
