...
let
  Architecture # :  struct { bits: integer, endianness: Endianness, family: Family }
    = type "Architecture" {
      ...
      from # :  type -> (string | Architecture)? -> Architecture
        =
          architecture:
          if !(Architecture.isInstanceOf architecture) then
            type.matchPrimitiveOrPanic architecture {
              null = all.unknown;
              string =
                all.${architecture} or (debug.panic "Architecture" "Unknown architecture »${architecture}«!");
            }
          else
            architecture;
    };
  ...
  Family # :  type
    = type "Family" { }; # { name = string; arch = string || null; version = integer || null; };
  ...
  families # :  { string -> Family }
    = {
      ...
      arm7-a = families.arm7 // {
        arch = "armv7-a";
      };
      arm7-m = families.arm7 // {
        arch = "armv7-m";
      };
      arm7-r = families.arm7 // {
        arch = "armv7-r";
      };
      arm8 = families.arm // {
        arch = "armv8";
      ...
      arm8-a = families.arm8 // {
        arch = "armv8-a";
      };
      arm8-m = families.arm8 // {
        arch = "armv8-m";
      };
      avr = {
        name = "avr";
