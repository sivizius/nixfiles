...
let
      ...
      result =
        list.fold
          (
            ...
            name:
            if self.${name} or null != null then
              if self.${name} ? id then
                ...
                // {
                  ...
                  text =
                    if text == null then
                      "${string self.${name}.id}"
                    else if previous != null && self.${name}.id == (previous + 1) then
                      text
                    else if range then
          ...
          citations;
      text = if result.text != null then result.text else "[???]";
    in
    if result.encode == [ ] then
      "\\textsuperscript{[${text}]}"
    else
  ...
  citeString =
    self: name:
    if self.${name} or null != null then
      if self.${name}.id or null != null then
