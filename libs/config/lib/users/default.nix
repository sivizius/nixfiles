...
let
  ...
  loadKey =
    ...
    let
      convert =
        ...
        let
          contact' = if contact != null then " contact = \"${contact}\";" else "";
          date' = time date;
    ...
    in
    if path.isInstanceOf key then convert (path.import key) else convert key;
        ...
        user = {
          ...
          keys = set.map (
            ...
            if hostKeys == null then
              [ ]
            else if list.isInstanceOf hostKeys then
    ...
    call =
      ...
      let
        environment' = environment // {
          inherit packages;
          user = user // {
            extra = set.mapValues (extra: extra."${host.system}" or extra) user.extra;
          };
          ...
          // {
            config =
              if hasHomeManager then
                ...
                debug.error "UserConfiguration" "Empty `config`, because home-manager is missing." { };
          }
        );
      };
    wrap =
      ...
      // (
        if hasHomeManager then
          {
            home-manager.users.${user.name} = set.remove configuration [ "host" ];
          }
        else
          debug.error "UserConfiguration" "Module `home-manager` is missing." { }
      );
  };
  ...
  load = source: environment: configurations.load source environment constructors User;

  prepare =
    environment: host: users:
    if set.isInstanceOf users then
      set.map (prepareUser environment host) users
    else
      debug.panic "prepare" "The option `users` must be a set.";

  prepareUser =
    environment: host: name: user:
    {
