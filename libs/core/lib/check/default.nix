...
let
  TestCase = type "TestCase"
    {
      from = { success, value } @ result:
        TestCase.instantiate result;
    };
  ...
  TestResult = type "TestResult"
    {
      from = variant:
        ...
        TestResult.instantiateAs variant result;
    };
  ...
  testsToChecks = { fileName, lib, source, tests }:
    buildSystem:
    arguments:
    let
