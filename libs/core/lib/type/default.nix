...
let
  TypeConstructor = type "TypeConstructor"
    {
      from =
        let
          __functor = { apply, instantiate, ... } @ self:
            argument:
            let
  ...
  defaultFunctor = { expect, from ? null, ... } @ self:
    value:
  ...
  defaultInstanceOf = this:
    value:
    get value == this;

  defaultMergeWith = variant:
    prev:
    next:
  ...
  enum =
    let
      constructEnum = { __variant__, ... } @ self:
        variantName:
        value:
        let
  ...
  # Get Type
  get/* :  T: Introspection @ T -> string */ = value:
    let
  ...
  getPrimitive/* :  T: Introspection @ T -> string */ = intrinsics.typeOf;
  ...
  matchPrimitive' = function "matchPrimitive"
    [ any any any ]
    matchPrimitive;
  ...
  matchPrimitiveOrPanic' = function "matchPrimitiveOrPanic"
    [ any any any ]
    matchPrimitiveOrPanic;

  struct/* :  string -> { string } -> type */ =
    let
      checkFields = structName:
        fields:
        signatures:
        let
