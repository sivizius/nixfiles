...
let
  ...
  report =
    ...
    let
      ...
      formatTemperature = temperature: formatValue' temperature "celsius";

      optional = value: text: if value != null then [ text ] else [ ];
...
in
{
  report = resources: value: if value != null then report value else [ ];
}
