...
let
      ...
      type.matchPrimitiveOrDefault value
        {
          ...
          string =
            ...
            in
            if result != null then result else debug.panic "from" "Could not convert string ${value} to int!";
        }
        (
          debug.panic "from" {
  ...
  toInteger # :  string -> int | null
    =
      ...
      in
      if isInstanceOf result then if sign == "-" then (0 - result) else result else null;
