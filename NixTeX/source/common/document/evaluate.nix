...
let
  ...
  evaluateLine =
    let
      ...
      evaluateToken =
        ...
        in
        if lambda.isInstanceOf state.${list.head token'} then
          state
          // {
            "${token'}" = { ... }: state.${list.head token'} state arguments;
          }
        else
          state
          // {
    ...
    in
    state: body:
    list.fold # Lists will be evaluated from left to right!
      (state: token: if list.isInstanceOf token then evaluateToken state token else state)
      state
  ...
  evaluate =
    ...
    type.matchPrimitiveOrPanic body {
      ...
      set =
        if body ? __type__ then
          (Chunk.expect body).evaluate document state body
        else
          #throw "Not a Chunk"
          debug.panic "evaluate" {
            text = "This is not a Chunk!";
            data = set.names body;
          };
      string = state;
    };
